...
def setup_calendar_diary_state_router(db):
    ...
    async def diary_state(
        ...
        # expected_entries[date_iso] = [ {entry skeleton}, ... ]
        expected_by_date: dict = {}
        for a in assignments:
            for slot in a.get("weekly_slots", []) or []:
                wd = slot.get("weekday")  # 1=Seg..7=Dom
                ...
                for day in _daterange(d_from, d_to):
                    if not _is_assignment_active_on(a, day):
                        continue
                    py_wd = day.isoweekday()  # 1=Mon..7=Sun
                    if py_wd != wd:
                        continue
                    iso = day.isoformat()
                    expected_by_date.setdefault(iso, []).append({
                        "component_id": a.get("component_id"),
                        "component_name": a.get("component_id"),  # fallback; mapping real virá da Fase 9
                        "aula_numero": aula,
        ...
        # ---------------- Etapa 3: buscar evidências (attendance + content) ----------------
        ...
        # ---------------- Etapa 4: casamento em Python ----------------
        # Anos iniciais: 1 attendance por (class+date) sem aula_numero → aplica a TODOS slots do dia.
        # Anos finais: 1 attendance por (class+date+course+aula_numero) → match específico.
        # Index attendances por date e por (date, aula_numero):
        ...
        # Index content por (date, component_id, aula_numero, teacher_id)
        ce_index: dict = {}
        ...
        for iso, entries in expected_by_date.items():
            for e in entries:
                # Match por (date, aula_numero) — anos finais.
                ...
                if specific:
                    ...
                    # Fallback: attendance única do dia (anos iniciais) aplica a todos os slots.
                ...
                # Content: match por chave completa
        ...
        # ---------------- Detectar evidência órfã (fora de slot esperado) ----------------
