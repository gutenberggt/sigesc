...
- o PDF usa o gerador oficial ``generate_ficha_individual_pdf``;
- resultado/data entram apenas pelos overrides explícitos do gerador oficial;
- a única escrita deste fluxo é a trilha documental em ``manual_document_issuances``.
"""
...
# Contrato aprovado da ferramenta de contingência. Não ampliar sem nova decisão.
_ALLOWED_ROLES = {
    "super_admin",
    "admin",
    "admin_teste",
...
def _ensure_role_and_school(user: dict, school_id: str) -> None:
    ...
    if role not in _ALLOWED_ROLES:
        raise HTTPException(
            status_code=403,
            detail="Usuário sem permissão para acessar Urgências",
        )

    # Perfis administrativos globais deste contrato não dependem de school_links.
    if role in {"super_admin", "admin", "admin_teste"}:
        return
    ...
    *,
    ...
    if _is_multigrade(class_info):
        if not student_series:
            raise HTTPException(
                status_code=400,
                detail="Ano/Série/Etapa é obrigatório para turma multisseriada",
            )
        if (
            enrollment_series
            and enrollment_series.casefold() != student_series.strip().casefold()
        ):
            raise HTTPException(
                status_code=400,
                detail=(
                    f"A matrícula do estudante pertence a {enrollment_series}, "
                    f"não a {student_series}"
                ),
            )
        ...
        enrollment = dict(enrollment)
        enrollment["student_series"] = (
            class_info.get("grade_level") or student.get("student_series")
        )

    grade_level = enrollment.get("student_series") or class_info.get("grade_level") or ""
    nivel_ensino = (
        inferir_nivel_ensino(class_info, enrollment)
        or "fundamental_anos_iniciais"
    )
    ...
    if not courses:
        raise HTTPException(
            status_code=400,
            detail="Não foi possível resolver o currículo da turma selecionada",
        )
    ...
    calendario = await _load_calendar(
        db,
        academic_year,
        user=user,
        request=request,
    ...
    *,
    ...
    for event in events:
        ...
        while current <= end_date:
            ...
            elif current.weekday() == 5 and (
                event_type == "sabado_letivo"
                or event.get("is_school_day") is True
            ):
    ...
    calendario["dias_letivos_calculados"] = (
        total or calendario.get("dias_letivos_previstos") or 200
    )
    return calendario
        ...
        courses.append(
            {
                ...
                "frequency_percentage": attendance.get(
...
def _convert_manual_grades(
    payload: ManualFichaIn,
    ctx: dict[str, Any],
) -> list[dict[str, Any]]:
    ...
    if len(received_ids) != len(set(received_ids)):
        raise HTTPException(
            status_code=400,
            detail="Componente curricular duplicado no preenchimento",
        )
    invalid = [cid for cid in received_ids if cid not in valid_ids]
    if invalid:
        raise HTTPException(
            status_code=400,
            detail=f"Componente fora do currículo resolvido: {invalid[0]}",
        )
        ...
        for field in fields:
            ...
            if conceptual:
                ...
                if code not in allowed_concepts:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Conceito inválido para a etapa: {code}",
                    )
                numeric = conceito_para_valor(code)
                if numeric is None:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Conceito inválido: {code}",
                    )
                row[field] = numeric
            else:
                try:
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail=f"Nota inválida no campo {field}",
                    ) from exc
                if numeric < 0 or numeric > 10:
                    raise HTTPException(
                        status_code=400,
                        detail="As notas devem estar entre 0 e 10",
                    )
                row[field] = numeric
...
def setup_router(db, audit_service=None, sandbox_db=None, **kwargs):
    ...
    async def preview_manual_ficha(
        ...
        conceptual = _is_conceptual(
            ctx["nivel_ensino"], ctx["grade_level"]
        )
        return {
            "school": {
                "id": ctx["school"].get("id"),
                "name": ctx["school"].get("name"),
            },
            "class": {
            ...
            "student": {
                ...
                "inep_code": (
                    ctx["student"].get("inep_code")
                    or ctx["student"].get("inep_number")
                ),
            },
    ...
    async def generate_manual_ficha(payload: ManualFichaIn, request: Request):
        ...
        filename_base = (
            ctx["student"].get("full_name") or "estudante"
        ).replace(" ", "_")
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    'attachment; filename="'
                    f"ficha_individual_urgencia_{filename_base}.pdf"
                    '"'
                )
            },
        )
