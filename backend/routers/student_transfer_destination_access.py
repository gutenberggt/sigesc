...
def _assert_transfer_candidate_same_tenant(student_doc: dict, current_user: dict, request: Request) -> None:
...
def install_student_transfer_destination_access(
    ...
    async def transfer_aware_get_student(student_id: str, request: Request):
        ...
        if not student_doc:
            # Preserva a resposta/semântica 404 do endpoint legado.
            return await current_get(student_id, request)
    ...
    async def destination_guarded_update_student(
        ...
        if current_user.get("role") == "secretario":
            ...
            if student_doc:
                ...
                # outra mantenedora. Para status não transferido, a rota legada
