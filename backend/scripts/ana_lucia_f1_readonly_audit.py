...
Escopo autorizado em 2026-09-01 (issue #304): investigar exatamente os 17 pares
(turma, componente) informados para Ana Lucia Faria Pinto, ano letivo 2026,
...
ACADEMIC_YEAR = int(os.environ.get("ANA_LUCIA_F1_ACADEMIC_YEAR", "2026"))
REFERENCE_DATE = os.environ.get("ANA_LUCIA_F1_REFERENCE_DATE", date.today().isoformat())[:10]
...
async def _unique_teacher_identity(db) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if len(exact_users) == 1:
        ...
        if user is None and _sid(staff.get("email")):
            ...
            if len(matches) == 1:
                user = matches[0]
        if user is None:
            raise RuntimeError("ANA_LUCIA_F1_USER_IDENTITY_UNRESOLVED")
    else:
        raise RuntimeError(
            f"ANA_LUCIA_F1_USER_IDENTITY_AMBIGUOUS:users={len(exact_users)}:staff={len(exact_staff)}"
        )
    ...
    if not linked_staff:
        raise RuntimeError("ANA_LUCIA_F1_STAFF_IDENTITY_UNRESOLVED")
...
async def _run_live_audit() -> dict[str, Any]:
    ...
    if not mongo_url:
        raise RuntimeError("ANA_LUCIA_F1_MONGO_URL_MISSING")
    ...
    try:
        ...
        return {
            "schema": "ANA_LUCIA_F1_READ_ONLY_AUDIT_V1",
            "status": "PASS",
