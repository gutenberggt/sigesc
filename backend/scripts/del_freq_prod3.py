...
# Dias EXPLICITOS a remover (MM-DD). Fev: 13,19,20,23-27 | Mar: 02-06,09-13,16-20,23-26
DIAS_MMDD = [
    "02-13", "02-19", "02-20", "02-23", "02-24", "02-25", "02-26", "02-27",
...
async def main():
    ...
    if not year:
        ...
        return
    datas = ["%s-%s" % (year, mmdd) for mmdd in DIAS_MMDD]
    print("\n[DATAS] %d dias (ano letivo=%s): %s" % (len(datas), year, ", ".join(datas)))
    ...
    docs = await db.attendance.find({"class_id": turma["id"], "date": {"$in": datas}},
    ...
    print("\n[FREQUENCIA] docs nesses dias: %d | com registro da aluna: %d" % (len(docs), len(afet)))
    for aid, dt, st in afet:
        print("   - %s  status=%s  (attendance_id=%s)" % (dt, st, aid))
    faltando = sorted(set(datas) - {d.get("date") for d in docs})
    if faltando:
        print("   (dias SEM documento de frequencia nesta turma: %s)" % ", ".join(faltando))
    if not afet:
        print("\nNada a remover.")
        return
    if not APPLY:
