...
The collector is streamed into the already-running production backend container.
It compares, for the same class and months, three read projections:

1. direct MongoDB legacy records (all tenant states, target tenant, missing tenant);
2. authenticated professor HTTP GET /learning-objects (class-wide and per component);
3. authenticated super-admin HTTP GET /learning-objects, both unscoped and explicitly
   scoped to the class tenant.

It also maps the professor's legacy teacher_assignments to raw
teacher_class_assignments by component. This is diagnostic only: no assignment,
content, tenant or user record is mutated.

Privacy boundary:
- no content text is emitted;
- no learning-object, assignment, teacher, student or user id is emitted;
- no student data is read or emitted;
- ephemeral access tokens stay in process memory and are never persisted/emitted;
- only component names and structural counters are emitted.

Mutation boundary:
- MongoDB is read-only;
- HTTP methods are GET only;
- no login endpoint is called (login would write audit state);
- no restart/deploy/backfill is performed by this collector.
"""
...
def _month_bounds(month: int) -> tuple[str, str]:
    ...
    if month == 12:
        end = f"{ACADEMIC_YEAR + 1}-01-01"
    else:
        end = f"{ACADEMIC_YEAR}-{month + 1:02d}-01"
    return start, end


def _counts_by_component(rows: list[dict[str, Any]]) -> Counter[str]:
    return Counter(_sid(row.get("course_id") or row.get("component_id")) for row in rows if row.get("course_id") or row.get("component_id"))
...
def analyze_component_parity(
    *,
    ...
    """Classify component-level structural parity without inspecting content text."""
...
def _school_ids_for_professor(db, user: dict[str, Any], staff: dict[str, Any]) -> list[str]:
    ...
    for row in rows:
        ...
        if school_id and school_id not in ids:
    ...
    for link in user.get("school_links") or []:
        ...
        if school_id and (not roles or "professor" in roles) and school_id not in ids:
            ids.append(school_id)
    return ids


def _component_http_counts(rows: list[dict[str, Any]]) -> Counter[str]:
    return _counts_by_component(rows or [])
...
def run_live_audit() -> dict[str, Any]:
    ...
    school = _unique_one(
        db.schools, {"name": SCHOOL_NAME},
        {"_id": 0, "id": 1, "mantenedora_id": 1}, "SCHOOL",
    )
    teacher = _unique_one(
    ...
    staff_query: dict[str, Any] = {"$or": [{"user_id": teacher["id"]}]}
    ...
    if len(legacy_course_ids) != EXPECTED_COMPONENTS:
        # Still collect the state; classification will remain entitlement drift.
        pass

    course_docs = list(db.courses.find(
        {"id": {"$in": legacy_course_ids}},
        {"_id": 0, "id": 1, "name": 1},
    ))
    course_name_by_id = {_sid(row.get("id")): str(row.get("name") or "") for row in course_docs}
    unresolved_courses = [cid for cid in legacy_course_ids if cid not in course_name_by_id]
    if unresolved_courses:
        raise RuntimeError(f"P0_250_F2_6_UNRESOLVED_COURSES:{len(unresolved_courses)}")
    ...
    for row in dvd_rows:
        ...
        current = enabled and (not start or start <= today) and (not end or today <= end)
        if not current:
            continue
    ...
    # Confirm the professor HTTP entitlement names independently of Mongo mapping.
    ...
    for month in TARGET_MONTHS:
        ...
        superadmin_scoped_rows = _api_get(
            superadmin_token, "/learning-objects", params, extra_headers=scoped_headers
        ) or []
        per_month_http[month] = {
            "professor_classwide": _component_http_counts(professor_rows),
            "superadmin_unscoped": _component_http_counts(superadmin_unscoped_rows),
            "superadmin_scoped": _component_http_counts(superadmin_scoped_rows),
        }
    ...
    for course_id in sorted(legacy_course_ids, key=lambda cid: _norm(course_name_by_id[cid])):
        ...
        for month in TARGET_MONTHS:
            ...
            mongo_missing = db.learning_objects.count_documents({**base, "mantenedora_id": {"$exists": False}})
            mongo_null = db.learning_objects.count_documents({**base, "mantenedora_id": None})
            # Missing/null are reported together without tenant identities.
            missing_or_null = mongo_missing + mongo_null
            mongo_other = max(0, mongo_all - mongo_target - missing_or_null)
            ...
            months[str(month)] = {
                ...
                "mongo_missing_or_null_tenant": missing_or_null,
    ...
    analysis["professor_turmas_component_count"] = len(professor_http_course_ids)
    analysis["dvd_raw_total_rows"] = len(dvd_rows)
    analysis["dvd_raw_class_wide_rows"] = dvd_class_wide_rows
    analysis["dvd_enabled_current_class_wide_rows"] = dvd_enabled_current_class_wide
    analysis["reference_date"] = today
...
def main() -> None:
    ...
    print("P0_250_F2_6_AUDIT_JSON=" + json.dumps(payload, ensure_ascii=False, separators=(",", ":")))
