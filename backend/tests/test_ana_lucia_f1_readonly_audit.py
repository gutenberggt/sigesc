...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "ana_lucia_f1_readonly_audit.py"
spec = importlib.util.spec_from_file_location("ana_lucia_f1", SCRIPT)
mod = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(mod)


def test_authorized_target_has_exactly_17_pairs():
