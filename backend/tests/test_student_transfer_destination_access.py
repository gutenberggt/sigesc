...
@pytest.mark.asyncio
async def test_active_student_keeps_legacy_school_authorization(monkeypatch):
    seen = {}
...
@pytest.mark.asyncio
async def test_transfer_to_unlinked_destination_is_blocked_before_legacy_write(monkeypatch):
    seen = {}
    db = _DB(_student())
    router = _install_router(db, seen)
...
def test_installation_is_idempotent_and_wired_after_existing_student_adapters():
    seen = {}
    db = _DB(_student())
    router = _install_router(db, seen)
    ...
    assert "configured = install_student_enrollment_audit_semantics" in init_source
    assert "return install_student_transfer_destination_access(configured, db, sandbox_db)" in init_source
