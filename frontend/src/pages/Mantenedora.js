...
export default function Mantenedora() {
  ...
  // Upload do brasão (imagem única usada em todo o sistema)
  const handleBrasaoUpload = async (event) => {
    ...
    try {
      ...
      if (result.url) {
        ...
        const newFormData = { ...formData, brasao_url: result.url };
        ...
        try {
          ...
          refreshMantenedora();
          showAlert('success', 'Brasão salvo com sucesso!');
        } catch (saveError) {
          console.error('Erro ao salvar brasão:', saveError);
          showAlert('warning', 'Brasão enviado, mas houve erro ao salvar. Clique em Salvar novamente.');
        }
      }
    } catch (error) {
  ...
  const handleRemoveBrasao = async () => {
    const newFormData = { ...formData, brasao_url: '' };
    setFormData(newFormData);
    ...
    try {
      ...
      await mantenedoraAPI.update(dataToSend);
      refreshMantenedora();
      showAlert('success', 'Brasão removido com sucesso!');
    } catch (error) {
      console.error('Erro ao remover brasão:', error);
      showAlert('error', 'Erro ao remover brasão');
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Brasão (Imagem única do sistema) */}
              <div>
                <Label>Brasão / Logotipo</Label>
                <p className="text-xs text-gray-500 mb-2">Esta imagem será usada em todos os documentos e cabeçalhos do sistema.</p>
                <div className="mt-2">
                  {formData.brasao_url ? (
