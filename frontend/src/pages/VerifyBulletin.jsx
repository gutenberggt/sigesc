/**
 * Página PÚBLICA de verificação de Boletim Oficial.
 * SEM autenticação. Acessível por qualquer pessoa via QR Code.
 * Caminho: /verify/boletim/:token
 */
...
export default function VerifyBulletin() {
  ...
  useEffect(() => {
    ...
    // Sem Authorization header. Mas alguns interceptors do axios podem injetar
    // — então usamos uma instância isolada com headers vazios.
    const isolated = axios.create();
    isolated
      .get(`${API}/api/verify/boletim/${token}`)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-12 flex items-center justify-center">
      <div className="max-w-xl w-full">
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-slate-600 mt-1">Sistema SIGESC · Boletim Escolar Oficial</p>
        </div>
        ...
        {state.status === 'error' && (
          <Card className="border-red-200">
            <CardContent className="py-8 text-center">
              ...
              <div className="text-xs text-red-700 mt-2" data-testid="verify-error-detail">
                {state.detail}
              </div>
              <div className="text-xs text-slate-500 mt-4">
                Verifique se o QR Code foi lido corretamente ou se o link foi
                copiado por completo.
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {state.status === 'ok' && state.data?.valid === true && (
          <Card className="border-emerald-300 shadow-sm" data-testid="verify-ok-card">
            <CardContent className="py-6">
              <div className="flex items-center gap-3 pb-4 border-b border-emerald-100">
                ...
                <div>
                  <div className="text-base font-semibold text-emerald-900">Documento autêntico</div>
                  <div className="text-xs text-emerald-700">
                    Emitido oficialmente pelo SIGESC e válido na data desta consulta.
                  </div>
                </div>
              </div>
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 mt-5 text-sm">
                ...
                <Field label="Ano letivo" value={state.data.academic_year} />
                <Field label="Estudante(a)" value={state.data.student_name} />
                <Field label="Escola" value={state.data.school_name} />
                <Field label="Turma" value={state.data.class_name} />
                <Field label="Série / Etapa" value={state.data.grade_level || '—'} />
                <Field label="Emitido em" value={formatDate(state.data.issued_at)} />
                <Field label="Verification ID" value={(state.data.verification_id || '').slice(0, 12) + '…'} mono />
              </dl>
              ...
              <div className="mt-5 pt-4 border-t border-emerald-100">
                ...
                <code
                  className="block text-[10.5px] bg-slate-100 rounded px-2 py-1.5 break-all text-slate-700"
                  data-testid="verify-sha256"
                >
              ...
              <div className="mt-5 flex items-center gap-2 text-[11px] text-slate-500">
                ...
                Esta página não exibe notas detalhadas, dados pessoais sensíveis
                nem registros disciplinares — apenas dados-resumo para
                confirmação de autenticidade (LGPD).
              </div>
            </CardContent>
          </Card>
        )}
