...
function guideUrl(queryParam, slug) {
  return `/tutoriais?${queryParam}=${encodeURIComponent(slug)}`;
}

function TrailNav({ currentSlug, tutorials, categories, queryParam, trailTitle, trailDescription, theme }) {
  return (
    <aside className="lg:sticky lg:top-24 self-start">
      <div className="bg-slate-800/55 border border-slate-700/60 rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <ListChecks size={19} className={theme.accentText} />
          <h2 className="font-semibold text-white">{trailTitle}</h2>
        </div>
        <p className="text-xs leading-5 text-slate-400 mb-5">{trailDescription}</p>

        <div className="space-y-5">
          {categories.map((category) => {
            const items = tutorials.filter((item) => item.category === category);
            return (
              <div key={category}>
                ...
                <div className="space-y-1">
                  {items.map((item) => {
                    ...
                    return (
                      <Link
                        key={item.slug}
                        to={guideUrl(queryParam, item.slug)}
...
function NotFoundTutorial({ theme }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center px-4">
      <div className="max-w-lg text-center">
        <BookOpen size={52} className={`${theme.accentText} mx-auto mb-5`} />
        ...
        <Link
          to="/tutoriais"
          className={`inline-flex items-center gap-2 rounded-xl px-5 py-3 font-medium text-white transition-colors ${theme.primaryButton}`}
        >
          <ArrowLeft size={18} />
...
export default function TutorialRoleGuide({
  slug,
  tutorials,
  tutorialBySlug,
  categories,
  ...
  if (!tutorial) return <NotFoundTutorial theme={theme} />;

  const currentIndex = tutorials.findIndex((item) => item.slug === slug);
  const previous = currentIndex > 0 ? tutorials[currentIndex - 1] : null;
  ...
  const categoryStyle = categoryStyles[tutorial.category] || categoryStyles[categories[0]];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="h-1 bg-slate-800">
        <div
          className={`h-full bg-gradient-to-r transition-all ${theme.progress}`}
          ...
          <article className="min-w-0">
            ...
            <section className="mb-8">
              <div className="flex flex-wrap items-center gap-3 mb-5">
                <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium ${categoryStyle.badge}`}>
                ...
                <span className="text-xs text-slate-600">
                  Guia {currentIndex + 1} de {tutorials.length}
                </span>
              </div>
            ...
            <section className="grid md:grid-cols-[1fr_auto] gap-4 mb-8">
              <div className={`rounded-2xl border p-5 sm:p-6 ${theme.objectiveBox}`}>
                <div className="flex gap-3 items-start">
                  <Target size={22} className={`${theme.accentText} flex-shrink-0 mt-0.5`} />
                  <div>
                    <p className={`text-sm font-semibold mb-1 ${theme.accentTextStrong}`}>Objetivo deste guia</p>
              ...
              <Link
                ...
                className={`rounded-2xl border border-slate-700 bg-slate-800/70 px-5 py-4 flex md:flex-col items-center justify-center gap-2 text-sm font-medium text-slate-200 hover:text-white transition-colors ${theme.resourceHover}`}
              >
                <ExternalLink size={19} className={theme.accentText} />
            ...
            <section className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-5 sm:p-6 mb-8">
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <h2 className="font-semibold text-white mb-2">{mindsetTitle}</h2>
                  <p className="text-sm leading-6 text-slate-400">{mindsetText}</p>
                </div>
              </div>
            </section>
                ...
                {tutorial.steps.map((step, index) => (
                  <div key={step.title} className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5 sm:p-6">
                    <div className="flex gap-4 items-start">
                      <div className={`w-9 h-9 rounded-full border flex items-center justify-center font-bold flex-shrink-0 ${theme.stepCircle}`}>
                        {index + 1}
                      </div>
                      <div className="min-w-0">
            ...
            <section className={`rounded-2xl border p-6 sm:p-7 mb-10 ${theme.completionBox}`}>
              <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                <CheckCircle2 size={21} className={theme.accentText} />
                Você concluiu este guia quando...
              </h2>
              <div className="space-y-3">
                {tutorial.doneWhen.map((item) => (
                  <div key={item} className="flex gap-3 items-start">
                    <div className={`mt-1 w-5 h-5 rounded bg-slate-950/40 flex-shrink-0 ${theme.checkbox}`} />
            ...
            <nav className="grid sm:grid-cols-2 gap-4 border-t border-slate-800 pt-7" aria-label="Tutoriais anterior e próximo">
              {previous ? (
                <Link
                  to={guideUrl(queryParam, previous.slug)}
                  className={`group rounded-2xl border border-slate-800 bg-slate-900/45 p-5 transition-colors ${theme.navHover}`}
                >
                  <span className="text-xs text-slate-500 flex items-center gap-1 mb-2">
                    <ArrowLeft size={14} /> Anterior
                  </span>
                  <span className="text-sm font-medium text-slate-300 group-hover:text-white">{previous.title}</span>
                </Link>
              ) : <div />}

              {next ? (
                <Link
                  to={guideUrl(queryParam, next.slug)}
                  className={`group rounded-2xl border border-slate-800 bg-slate-900/45 p-5 text-right transition-colors ${theme.navHover}`}
                >
                  <span className="text-xs text-slate-500 flex items-center justify-end gap-1 mb-2">
                    Próximo <ArrowRight size={14} />
                  </span>
                  <span className="text-sm font-medium text-slate-300 group-hover:text-white">{next.title}</span>
                </Link>
              ) : (
                <Link
                  to="/tutoriais"
                ...
                >
                  <span className="text-xs text-slate-500 flex items-center justify-end gap-1 mb-2">
                    Concluir trilha <ArrowRight size={14} />
                  </span>
                  <span className="text-sm font-medium text-slate-300 group-hover:text-white">Voltar à Central de Tutoriais</span>
                </Link>
              )}
            </nav>
          </article>
