...
- **Correção FINAL (regras do usuário)** — substitui a tentativa de dedupe cross-turma (revertida):
  - `services/pedagogical_consolidation.py` (regra #1): migração origem→destino agora faz **merge-fill** (preenche só campos VAZIOS do destino com valores da origem; nunca sobrescreve; marca migrado/congelado). Antes PULAVA quando havia registro no destino.
  - `routers/documents.py` (regra #4): Boletim (prefere matrícula `active`) e Ficha leem **somente a turma ativa/destino**. Dedupe cross-turma REMOVIDO.
  - `routers/history_reconstruction.py`: dry-run passou a contar também campos vazios que o merge preencheria.
  - Backfill dos 118 alunos: ferramenta **Reconstrução** (`/admin/reconstrucao-historico`) reusa a migração merge.
