    ...
    try {
      ...
      if (nomeCompletoSearch.trim() || cpfSearch.trim()) {
        ...
        if (nomeCompletoSearch.trim()) {
          pessoaQueryConstraints.push(
            where("nomeCompleto", ">=", nomeCompletoSearch.trim().toUpperCase())
          );
          pessoaQueryConstraints.push(
            where(
              "nomeCompleto",
              "<=",
              nomeCompletoSearch.trim().toUpperCase() + "\uf8ff"
            )
          );
        }
        if (cpfSearch.trim()) {
          pessoaQueryConstraints.push(
            where("cpf", "==", cpfSearch.replace(/\D/g, ""))
          );
        }
        ...
        if (pessoaQueryConstraints.length > 0) {
          ...
          if (pessoaIdsFinal.length === 0) {
            setSearchErrorMessage(
              "Nenhum aluno encontrado com os dados pessoais informados."
            );
      ...
      if (pessoaIdsFinal !== null) {
        ...
        if (pessoaIdsFinal.length > 30) {
          setSearchErrorMessage(
            "A busca por nome/CPF retornou muitos resultados. Refine a busca ou adicione filtros de matrícula (escola, turma, etc)."
          );
          setIsSearching(false);
          return;
        }
      ...
      if (situacaoSearch)
        finalQueryConstraints.push(
          where("situacaoMatricula", "==", situacaoSearch)
        );

      if (finalQueryConstraints.length === 0) {
        setSearchErrorMessage(
          "Por favor, preencha ao menos um campo para realizar a busca."
        );
        setIsSearching(false);
        return;
      }
      ...
      const resultsData = await Promise.all(
        matriculasSnapshot.docs.map(async (docMatricula) => {
          const matriculaData = docMatricula.data();
          if (!matriculaData.pessoaId || !matriculaData.escolaId) return null;

          const pessoaDocSnap = await getDoc(
            doc(db, "pessoas", matriculaData.pessoaId)
          );
          const escolaDocSnap = await getDoc(
            doc(db, "schools", matriculaData.escolaId)
          );
          const turmaDocSnap = matriculaData.turmaId
            ? await getDoc(doc(db, "turmas", matriculaData.turmaId))
            : null;
          ...
          return {
            ...
            nomeEscola: escolaDocSnap.exists()
              ? escolaDocSnap.data().nomeEscola
              : "N/A",
            nomeTurma: turmaDocSnap?.exists()
              ? turmaDocSnap.data().nomeTurma
              : "N/A",
          };
        })
      );
      ...
      setSearchErrorMessage(
        "Ocorreu um erro inesperado ao realizar a busca. Tente novamente."
      );
    } finally {
      setIsSearching(false);
    }
    ...
    <div className="flex-grow p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-full mx-auto">
        ...
        <form
          ...
          className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 border rounded-md bg-gray-50 mb-6"
        >
          <div className="col-span-full md:col-span-2 lg:col-span-3">
            <label
              htmlFor="nomeCompletoSearch"
            ...
            <input
              type="text"
              id="nomeCompletoSearch"
              className="mt-1 block w-full p-2 border rounded-md"
              value={nomeCompletoSearch}
              onChange={(e) => setNomeCompletoSearch(e.target.value)}
              autoComplete="off"
            />
          </div>
          ...
          <div className="col-span-full md:col-span-2 lg:col-span-2">
            ...
            <select
              id="escolaSearchId"
              className="mt-1 block w-full p-2 border rounded-md"
              value={escolaSearchId}
              onChange={(e) => setEscolaSearchId(e.target.value)}
              autoComplete="off"
            >
          ...
          <div className="col-span-full md:col-span-1 lg:col-span-2">
            <label
              htmlFor="nivelEnsinoSearch"
              className="block text-sm font-medium text-gray-700"
            >
              Nível Ensino
            </label>
            <select
              id="nivelEnsinoSearch"
              className="mt-1 block w-full p-2 border rounded-md"
              value={nivelEnsinoSearch}
              onChange={(e) => setNivelEnsinoSearch(e.target.value)}
              disabled={!escolaSearchId}
              autoComplete="off"
            >
          ...
          <div className="col-span-full md:col-span-1 lg:col-span-1">
            ...
            <select
              id="anoSerieSearch"
              className="mt-1 block w-full p-2 border rounded-md"
              value={anoSerieSearch}
              onChange={(e) => setAnoSerieSearch(e.target.value)}
              disabled={!nivelEnsinoSearch}
              autoComplete="off"
            >
          ...
          <div className="col-span-full md:col-span-1 lg:col-span-1">
            ...
            <select
              ...
              className="mt-1 block w-full p-2 border rounded-md"
              value={turmaSearchId}
              onChange={(e) => setTurmaSearchId(e.target.value)}
              disabled={!anoSerieSearch}
              autoComplete="off"
            >
              <option value="">Todas</option>
        ...
        {isSearching ? (
          <p className="text-center text-blue-600">Buscando...</p>
                ...
                {searchResults.map((result) => (
                  <tr key={result.id} className="text-center">
                    <td className="border p-2 text-left">
                      <Link
                        to={`/dashboard/escola/aluno/ficha/${result.pessoaId}`}
          ...
          <p className="text-center text-gray-500">
            {searchErrorMessage ||
              "Nenhum resultado para exibir. Refine sua busca."}
          </p>
        )}
      </div>
    </div>
