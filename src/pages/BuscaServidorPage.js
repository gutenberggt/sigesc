...
function BuscaServidorPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      let servidoresList = servidoresSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      ...
      if (escolaId) {
        filteredList = filteredList.filter((s) =>
          s.alocacoes.some((a) => a.schoolId === escolaId)
        );
      }
      if (anoLetivo) {
        filteredList = filteredList.filter((s) =>
          s.alocacoes.some((a) => a.anoLetivo === anoLetivo)
        );
      }
      if (funcao) {
        filteredList = filteredList.filter((s) =>
          s.alocacoes.some((a) => a.funcoes.some((f) => f.funcao === funcao))
        );
      }
      if (turmaId) {
        filteredList = filteredList.filter((s) =>
          s.alocacoes.some((a) => a.funcoes.some((f) => f.turmaId === turmaId))
        );
      }
      if (nivelEnsino) {
        filteredList = filteredList.filter((s) =>
          s.alocacoes.some((a) =>
            a.funcoes.some((f) => f.niveisEnsino.includes(nivelEnsino))
          )
        );
      }
      if (serieAno && !turmaId) {
        const turmasComSerieQuery = query(
          collection(db, "turmas"),
          where("anoSerie", "==", serieAno)
        );
        ...
        if (turmasIdsComSerie.length > 0) {
          filteredList = filteredList.filter((s) =>
            s.alocacoes.some((a) =>
              a.funcoes.some((f) => turmasIdsComSerie.includes(f.turmaId))
            )
          );
        } else {
          filteredList = [];
      ...
      if (filteredList.length > 0) {
        const finalPessoaIds = [
          ...new Set(filteredList.map((s) => s.pessoaId)),
        ];
        if (finalPessoaIds.length > 0) {
          const pessoasQuery = query(
            collection(db, "pessoas"),
            where(documentId(), "in", finalPessoaIds)
          );
          const pessoasSnapshot = await getDocs(pessoasQuery);
          const pessoasMap = new Map();
          pessoasSnapshot.docs.forEach((doc) =>
            pessoasMap.set(doc.id, doc.data())
          );
                ...
                searchResults.map((res) => (
                  <tr key={res.id} className="border-b hover:bg-gray-100">
                    <td className="py-3 px-6 text-left">
                      <Link
                        to={`/dashboard/escola/servidor/ficha/${res.id}`}
                        className="text-blue-600 hover:underline"
                      >
                        {res.pessoa?.nomeCompleto}
                      </Link>
                    </td>
                    <td className="py-3 px-6 text-left">{res.pessoa?.cpf}</td>
                    <td className="py-3 px-6 text-left">
                      {res.alocacoes?.[0]?.funcoes?.[0]?.funcao || "N/A"}
                    </td>
                  </tr>
                ))
              ) : (
