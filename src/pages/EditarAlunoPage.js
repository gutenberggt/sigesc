...
function EditarAlunoPage() {
  ...
  const fetchAndSetAlunoData = useCallback(async () => {
    ...
    try {
      ...
      if (pessoaDocSnap.exists()) {
        ...
        setCpf(pessoaData.cpf || "");
        setNomeSocialAfetivo(pessoaData.nomeSocialAfetivo || "");
        setSexo(pessoaData.sexo || "Nao Informado");
        setEstadoCivil(pessoaData.estadoCivil || "Solteiro(a)");
        setDataNascimento(pessoaData.dataNascimento || "");
        setNacionalidade(pessoaData.nacionalidade || "");
        setRaca(pessoaData.raca || "Nao Declarada");
        setPovoIndigena(pessoaData.povoIndigena || "");
        setNaturalidadeCidade(pessoaData.naturalidadeCidade || "");
        setNaturalidadeEstado(pessoaData.naturalidadeEstado || "");
        setFalecido(pessoaData.falecido ? "sim" : "nao");
        setPessoaMae(pessoaData.pessoaMae || "");
        setTelefoneMae(pessoaData.telefoneMae || "");
        setEscolaridadeMae(pessoaData.escolaridadeMae || "Nao Informado");
        setProfissaoMae(pessoaData.profissaoMae || "");
        setPessoaPai(pessoaData.pessoaPai || "");
        setTelefonePai(pessoaData.telefonePai || "");
        setEscolaridadePai(pessoaData.escolaridadePai || "Nao Informado");
        setProfissaoPai(pessoaData.profissaoPai || "");
        setRgNumero(pessoaData.rgNumero || "");
        setRgDataEmissao(pessoaData.rgDataEmissao || "");
        setRgOrgaoEmissor(pessoaData.rgOrgaoEmissor || "");
        setRgEstado(pessoaData.rgEstado || "");
        setNisPisPasep(pessoaData.nisPisPasep || "");
        setCarteiraSUS(pessoaData.carteiraSUS || "");
        setCertidaoTipo(pessoaData.certidaoTipo || "Nascimento");
        setCertidaoNumero(pessoaData.certidaoNumero || "");
        setCertidaoEstado(pessoaData.certidaoEstado || "");
        setCertidaoCartorio(pessoaData.certidaoCartorio || "");
        setCertidaoDataEmissao(pessoaData.certidaoDataEmissao || "");
        setCertidaoCidade(pessoaData.certidaoCidade || "");
        setPassaporteNumero(pessoaData.passaporteNumero || "");
        setPassaportePaisEmissor(pessoaData.passaportePaisEmissor || "");
        setPassaporteDataEmissao(pessoaData.passaporteDataEmissao || "");
        setCep(pessoaData.cep || "");
        setRua(pessoaData.enderecoLogradouro || "");
        setEnderecoNumero(pessoaData.enderecoNumero || "");
        setEnderecoComplemento(pessoaData.enderecoComplemento || "");
        setEnderecoBairro(pessoaData.enderecoBairro || "");
        setMunicipioResidencia(pessoaData.municipioResidencia || "");
        setPaisResidencia(pessoaData.paisResidencia || "");
        setZonaResidencia(pessoaData.zonaResidencia || "urbana");
        setLocalizacaoDiferenciada(pessoaData.localizacaoDiferenciada || "");
        setPontoReferencia(pessoaData.pontoReferencia || "");
        setTelefoneResidencial(pessoaData.telefoneResidencial || "");
        setCelular(pessoaData.celular || "");
        setTelefoneAdicional(pessoaData.telefoneAdicional || "");
        setEmailContato(pessoaData.emailContato || "");
      } else {
      ...
      if (!matriculasSnapshot.empty) {
        ...
        setFotoPreview(matriculaData.fotoURL || "");
        setCodigoINEP(matriculaData.codigoINEP || "");
        setDataMatricula(matriculaData.dataMatricula || "");
        setMatriculaEscolaId(matriculaData.escolaId || "");
        setMatriculaNivelEnsino(matriculaData.nivelEnsino || "");
        setMatriculaAnoSerie(matriculaData.anoSerie || "");
        setMatriculaTurmaId(matriculaData.turmaId || "");
        setPessoasAutorizadas(matriculaData.pessoasAutorizadasBuscar || []);
        setBeneficiosSociais(matriculaData.beneficiosSociais || "");
        setDeficienciasTranstornos(matriculaData.deficienciasTranstornos || "");
        setReligiao(matriculaData.religiao || "");
        setUtilizaTransporte(matriculaData.utilizaTransporte ? "sim" : "nao");
        setVeiculoTransporte(matriculaData.veiculoTransporte || "");
        setRotaTransporte(matriculaData.rotaTransporte || "");
        setObservacoes(matriculaData.observacoes || "");
        setResponsavelLegalNome(
          matriculaData.responsavelLegalNome || pessoaData.pessoaMae || ""
        );
        setResponsavelLegalParentesco(
          matriculaData.responsavelLegalParentesco || ""
        );
      } else {
        setError("Este aluno não possui uma matrícula para edição.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (matriculaNivelEnsino) {
      ...
      const filteredSeries = seriesForLevel.filter((serie) =>
        schoolAnosSeriesData.includes(serie)
      );
  ...
  const handleGravar = async (e) => {
    ...
    const pessoaDataToSave = {
      ...
      telefoneMae: formatTelefone(telefoneMae).replace(/\D/g, ""),
      escolaridadeMae,
      profissaoMae: profissaoMae.toUpperCase(),
      pessoaPai: pessoaPai.toUpperCase(),
      telefonePai: formatTelefone(telefonePai).replace(/\D/g, ""),
      escolaridadePai,
      ...
      telefoneResidencial: formatTelefone(telefoneResidencial).replace(
        /\D/g,
        ""
      ),
      celular: formatTelefone(celular).replace(/\D/g, ""),
      telefoneAdicional: formatTelefone(telefoneAdicional).replace(/\D/g, ""),
      emailContato,
    ...
    try {
      ...
      if (fotoUpload) {
        const storageRef = ref(
          storage,
          `fotos_alunos/${alunoId}/${fotoUpload.name}`
        );
  ...
  if (loading)
    return (
      <div className="p-6 text-center">Carregando dados para edição...</div>
    );
    ...
    <div className="p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">
          Editar Dados de: <span className="text-blue-600">{nomeCompleto}</span>
        </h2>

        {successMessage && (
          <p className="text-green-600 bg-green-100 p-3 rounded-md text-sm mb-4 text-center">
            {successMessage}
          </p>
        )}
        {error && (
          <p className="text-red-600 bg-red-100 p-3 rounded-md text-sm mb-4 text-center">
            {error}
          </p>
        )}
        ...
        <form onSubmit={handleGravar} className="space-y-8">
          ...
          <fieldset className="border p-4 rounded-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mt-2">
              <div className="space-y-4">
                <h4 className="font-semibold text-center text-gray-600 border-b pb-2">
                  Dados da Mãe
                </h4>
                <div>
          ...
          <fieldset className="border p-4 rounded-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-2">
              <div className="col-span-full md:col-span-2">
                <label className="block text-sm font-medium">RG (Número)</label>
                <input
                  type="text"
                  value={rgNumero}
              ...
              <div className="col-span-full md:col-span-2">
                <label className="block text-sm font-medium">RG (Estado)</label>
                <input
                  type="text"
                  value={rgEstado}
              ...
              <div className="col-span-full md:col-span-2">
                <label className="block text-sm font-medium">Cartão SUS</label>
                <input
                  type="text"
                  value={carteiraSUS}
          ...
          <fieldset className="border p-4 rounded-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-2">
              ...
              <div className="col-span-full md:col-span-2">
                <label className="block text-sm font-medium">Série/Ano</label>
                <select
            ...
            <ul className="mt-4 space-y-2">
              {pessoasAutorizadas.map((p, i) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => handleRemovePessoaAutorizada(i)}
          ...
          <fieldset className="border p-4 rounded-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
              <div>
                <label className="block text-sm font-medium">Utiliza?</label>
                <select
          ...
          <div className="flex justify-end space-x-4 mt-8">
            ...
            <button
              ...
              className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded transition disabled:bg-green-300"
            >
