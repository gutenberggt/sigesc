...
function EditarServidorPage() {
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      ...
      const alocacoesComNomes = servidorData.alocacoes.map((aloc) => ({
        ...aloc,
        escolaNome:
          schoolsList.find((s) => s.id === aloc.schoolId)?.nomeEscola ||
          "Desconhecida",
      }));
  ...
  const handleAddFuncao = () => {
    ...
    const newFuncao = {
      ...
      componentesCurriculares:
        currentFuncao === "Professor(a)"
          ? selectedComponentes.map((c) => c.nome)
          : [],
    };
  ...
  const handleAddOrUpdateAlocacao = () => {
    if (!currentEscolaId || !currentAnoLetivo || currentFuncoes.length === 0) {
      setError("Preencha Escola, Ano Letivo e adicione pelo menos uma função.");
      return;
    }
    const alocacaoData = {
  ...
  const handleEditAlocacao = (index) => {
    ...
    const primeiraFuncaoProfessor = aloc.funcoes.find(
      (f) => f.funcao === "Professor(a)"
    );
    if (primeiraFuncaoProfessor) {
      ...
      const componentesSalvos =
        primeiraFuncaoProfessor.componentesCurriculares || [];
      const componentesObj = allComponentes.filter((comp) =>
        componentesSalvos.includes(comp.nome)
      );
      setSelectedComponentes(
        componentesObj.map((c) => ({ id: c.id, nome: c.nome }))
      );
    } else {
      setCurrentFuncao("");
      setCurrentTurmaId("");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(
        () => navigate(`/dashboard/escola/servidor/ficha/${servidorId}`),
        2000
      );
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (
      window.confirm(
        "ATENÇÃO: Isso excluirá o registro de servidor. Deseja continuar?"
      )
    ) {
      setIsSubmitting(true);
      try {
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        ...
        <div className="p-4 border-2 border-dashed rounded-md bg-gray-50 mb-6">
          ...
          <div className="flex justify-end space-x-2">
            ...
            <button
              type="button"
              onClick={handleAddOrUpdateAlocacao}
              className="bg-green-500 text-white py-2 px-4 rounded w-full"
            >
              {editingAlocacaoIndex !== null
                ? "Salvar Alterações na Alocação"
                : "Adicionar Alocação"}
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          {alocacoes.map((aloc, i) => (
            <div
              key={i}
              className="p-3 border rounded-md mb-2 bg-gray-100 flex justify-between items-center"
            >
              <div>
              ...
              <div className="space-x-2">
                <button
                  onClick={() => handleEditAlocacao(i)}
                  className="text-blue-600 text-sm"
                >
                  Editar
                </button>
                <button
                  onClick={() => handleRemoveAlocacao(i)}
