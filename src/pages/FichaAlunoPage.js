...
import { doc, getDoc, collection, query, where, getDocs, documentId, orderBy } from 'firebase/firestore';
...
const fichaConfig = {
    dadosPessoais: {
        ...
        fields: [
            { label: 'Nome', accessor: data => data.aluno?.nomeCompleto, colSpan: { md: 4, lg: 4 } },
            { label: 'CPF', accessor: data => data.aluno?.cpf, colSpan: { md: 2, lg: 2 } },
            { label: 'Data/Nasc.', accessor: data => data.aluno?.dataNascimento, format: formatDate, colSpan: { md: 1, lg: 1 } },
            { label: 'Sexo', accessor: data => data.aluno?.sexo, colSpan: { md: 1, lg: 1 } },
            { label: 'Est. Civil', accessor: data => data.aluno?.estadoCivil, colSpan: { md: 1, lg: 1 } },
            { label: 'Nacional.', accessor: data => data.aluno?.nacionalidade, colSpan: { md: 1, lg: 1 } },
            { label: 'Naturalidade', accessor: data => `${data.aluno?.naturalidadeCidade || ''} - ${data.aluno?.naturalidadeEstado || ''}`, colSpan: { md: 2, lg: 2 } },
            { label: 'Raça', accessor: data => data.aluno?.raca, colSpan: { md: 2, lg: 2 } },
            { label: 'Religião', accessor: data => data.aluno?.religião, colSpan: { md: 2, lg: 2 } },
    ...
    endereco: {
        title: 'Endereço',
        fields: [
            { label: 'Endereço Completo', accessor: data => `${data.aluno?.enderecoLogradouro || ''}, ${data.aluno?.enderecoNumero || ''} - ${data.aluno?.enderecoBairro || ''}`, colSpan: { md: 4, lg: 4 } },
            { label: 'Município', accessor: data => data.aluno?.municipioResidencia, colSpan: { md: 2, lg: 2 } },
            { label: 'CEP', accessor: data => data.aluno?.cep, colSpan: { md: 1, lg: 1 } },
			{ label: 'Ponto de Referência', accessor: data => data.aluno?.pontodeReferencia, colSpan: { md: 5, lg: 5 } },
        ]
    },
        ...
        sections: {
            mae: {
                ...
                fields: [
                    ...
                    { label: 'Telefone', accessor: data => data.aluno?.telefoneMae, format: formatTelefone },
                    { label: 'Escolaridade', accessor: data => data.aluno?.escolaridadeMae },
            ...
            pai: {
                ...
                fields: [
                    ...
                    { label: 'Telefone', accessor: data => data.aluno?.telefonePai, format: formatTelefone },
                    { label: 'Escolaridade', accessor: data => data.aluno?.escolaridadePai },
    ...
    documentacao: {
        title: 'Documentação',
        fields: [
            { label: 'RG', accessor: data => data.aluno?.rgNumero, colSpan: { md: 2 } },
            { label: 'Órgão Emissor', accessor: data => `${data.aluno?.rgOrgaoEmissor || ''} - ${data.aluno?.rgEstado || ''}`, colSpan: { md: 1 } },
            { label: 'Data/Emissão', accessor: data => data.aluno?.rgDataEmissao, format: formatDate, colSpan: { md: 1 } },
            { label: 'NIS/PIS/PASEP', accessor: data => data.aluno?.nisPisPasep, colSpan: { md: 1 } },
            { label: 'Cartão do SUS', accessor: data => data.aluno?.carteiraSUS, colSpan: { md: 1 } },
        ]
    },
    contatos: {
        title: 'Contatos',
        fields: [
            { label: 'E-mail', accessor: data => data.aluno?.emailContato, colSpan: { md: 2 } },
            { label: 'Celular', accessor: data => data.aluno?.celular, format: formatTelefone, colSpan: { md: 2 } },
            { label: 'Telefone', accessor: data => data.aluno?.telefoneResidencial, format: formatTelefone, colSpan: { md: 2 } },
        ]
    }
};
...
function FichaAlunoPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const matriculasQuery = query(collection(db, 'matriculas'), where('pessoaId', '==', alunoId), orderBy('dataMatricula', 'desc'));
        const matriculasSnapshot = await getDocs(matriculasQuery);
        const matriculasList = matriculasSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        if (matriculasList.length > 0) {
          const principal = matriculasList[0];
          setMatriculaPrincipal(principal);
          const schoolIds = [...new Set(matriculasList.map(m => m.escolaId).filter(Boolean))];
          const turmaIds = [...new Set(matriculasList.map(m => m.turmaId).filter(Boolean))];
          
          const schoolsMap = new Map();
          if (schoolIds.length > 0) {
            const schoolsQuery = query(collection(db, 'schools'), where(documentId(), 'in', schoolIds));
            const schoolsSnapshot = await getDocs(schoolsQuery);
            schoolsSnapshot.docs.forEach(doc => schoolsMap.set(doc.id, doc.data()));
          }
          ...
          if (turmaIds.length > 0) {
              ...
              turmasSnapshot.docs.forEach(doc => turmasMap.set(doc.id, doc.data().nomeTurma));
          }
          const enrichedMatriculas = matriculasList.map(matricula => ({
            ...matricula,
            nomeEscola: schoolsMap.get(matricula.escolaId)?.nomeEscola || 'Não Informado',
  ...
  if (loading) return <div className="p-6 text-center">Carregando ficha do aluno...</div>;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div ref={printRef} className="bg-white p-4 sm:p-8">
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    <div className="flex-shrink-0 flex flex-col items-center">
                        {matriculaPrincipal?.fotoURL ? (
                            <img src={matriculaPrincipal.fotoURL} alt="Foto do Aluno" className="w-32 h-40 object-cover rounded-lg border-2 border-gray-200 shadow-md" />
                        ) : (
                        ...
                        <div className="mt-4 text-center">
                            ...
                            <span>{matriculaPrincipal?.codigoAluno || 'Não Informado'}</span>
                        </div>
                    </div>
                    <div className="flex-grow">
                        <h3 className="text-xl font-bold mb-4 text-gray-700 border-b pb-2">{dadosPessoais.title}</h3>
                            ...
                            {dadosPessoais.fields.map(field => {
                                ...
                                return (
                                    <div key={field.label} className={colSpanClasses}>
                                        <span className="font-semibold text-gray-600">{field.label}:</span> {formattedValue || 'Não Informado'}
                                    </div>
                                );
                            })}
                        </div>
            ...
            {Object.values(otherSections).map(section => (
                <div key={section.title} className="bg-white p-8 rounded-lg shadow-md mb-2">
                    <h3 className="text-xl font-bold mb-4 text-gray-700 border-b pb-2">{section.title}</h3>
                    {section.isSectioned ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1">
                            {Object.values(section.sections).map(subSection => (
                                <div key={subSection.title} className="space-y-2">
                                    <h4 className="text-lg font-semibold text-gray-600">{subSection.title}</h4>
                                    {subSection.fields.map(field => {
                                        const value = field.accessor(allData);
                                        const formattedValue = field.format ? field.format(value) : value;
                                        return <p key={field.label}><span className="font-semibold">{field.label}:</span> {formattedValue || 'Não Informado'}</p>
                                    })}
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-white p-8 rounded-lg shadow-md">
                <h3 className="text-xl font-bold mb-4 text-gray-700 border-b pb-2">Histórico de Matrículas</h3>
                        ...
                        <tbody className="text-gray-600 text-sm font-light">
                            {matriculas.map(m => (
                                <tr key={m.id} className="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td className="py-3 px-6 text-left">{m.situacaoMatricula}</td>
                                    <td className="py-3 px-6 text-left">{formatDate(m.dataMatricula)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
                <div className={`flex justify-end space-x-4 mt-6 ${isExporting ? 'hidden' : ''}`}>
                    <button onClick={() => navigate(`/dashboard/escola/aluno/editar/${alunoId}`)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition">Editar Aluno</button>
                    <button onClick={() => navigate(`/dashboard/escola/aluno/nova-matricula/${alunoId}`)} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded transition">Nova Matrícula</button>
                </div>
            </div>
