...
function FichaServidorPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (servidorData.alocacoes && servidorData.alocacoes.length > 0) {
        const schoolIds = [
          ...new Set(servidorData.alocacoes.map((aloc) => aloc.schoolId)),
        ];
        ...
        if (schoolIds.length > 0) {
          ...
          schoolsSnapshot.docs.forEach((doc) =>
            schoolsMap.set(doc.id, doc.data().nomeEscola)
          );
        }
      ...
      const exportPDF = async () => {
        ...
        while (yOffset < scaledCanvasHeight) {
          ...
          const sliceHeight = Math.min(
            pageContentHeight,
            scaledCanvasHeight - yOffset
          );

          // Converte a altura da fatia em mm para pixels do canvas original
          const sliceHeightInCanvasPixels =
            (sliceHeight / pdfWidth) * canvasWidth;
          ...
          pageCtx.drawImage(
            canvas,
            0,
            yOffsetInCanvasPixels,
            canvasWidth,
            sliceHeightInCanvasPixels,
            0,
            0,
            canvasWidth,
            sliceHeightInCanvasPixels
  ...
  if (!servidor || !pessoa)
    return (
      <p className="p-6 text-center">Dados do servidor não encontrados.</p>
    );
      ...
      >
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p>
              <span className="font-semibold">Nome:</span> {pessoa.nomeCompleto}{" "}
              <span className="font-semibold ml-4">CPF:</span> {pessoa.cpf}
            </p>
            <p>
              <span className="font-semibold">Código Educacenso/Inep:</span>{" "}
              {pessoa.codigoINEP || "N/A"}{" "}
              <span className="font-semibold ml-4">Data de Nascimento:</span>{" "}
        ...
        <div className="border-t pt-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <p>
              ...
              {`${pessoa.enderecoLogradouro || ""}, ${pessoa.enderecoNumero || ""} - ${pessoa.enderecoBairro || ""}`}
            </p>
            <p>
              <span className="font-semibold">Município:</span>{" "}
              {`${pessoa.municipioResidencia || ""} - ${pessoa.naturalidadeEstado || ""}`}
            </p>
            <p>
              <span className="font-semibold">CEP:</span> {pessoa.cep}
            </p>
          </div>
        </div>
        ...
        <div className="border-t pt-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <p>
              <span className="font-semibold">Órgão Emissor:</span>{" "}
              {`${pessoa.rgOrgaoEmissor || ""} - ${pessoa.rgEstado || ""}`}
            </p>
            <p>
              <span className="font-semibold">Data de Emissão:</span>{" "}
              {formatDate(pessoa.rgDataEmissao)}
            </p>
            <p>
              <span className="font-semibold">NIS/PIS/PASEP:</span>{" "}
              {pessoa.nisPisPasep || "N/A"}
            </p>
            <p>
              <span className="font-semibold">Cartão do SUS:</span>{" "}
              {pessoa.carteiraSUS || "N/A"}
            </p>
          </div>
        </div>
        ...
        <div className="border-t pt-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <p>
              ...
              {pessoa.emailContato || "N/A"}
            </p>
            <p>
              <span className="font-semibold">Celular:</span>{" "}
              {formatTelefone(pessoa.celular)}
            </p>
          </div>
        </div>
        ...
        <div className="border-t pt-4">
          ...
          {servidor.alocacoes && servidor.alocacoes.length > 0 ? (
                    ...
                    {funcao.funcao === "Professor(a)" && (
                      <>
                        <p>
                          ...
                          {funcao.turmaNome || "Não especificada"}
                        </p>
                        <p>
                          <span className="font-semibold">
                            Níveis de Ensino:
                          </span>{" "}
                          {funcao.niveisEnsino?.join(", ") ||
                        ...
                        <div>
                          ...
                          <ul className="list-disc list-inside ml-4 text-sm">
                            {funcao.componentesCurriculares?.map((c) => (
                              <li key={c}>{c}</li>
                            ))}
                          </ul>
                        </div>
            ...
            <p className="text-gray-500">
              Nenhuma alocação encontrada para este servidor.
            </p>
          )}
        </div>
        ...
        {!isExporting && (
          <div className="flex justify-end space-x-2 mt-6">
            <button
              onClick={() =>
                navigate(`/dashboard/escola/servidor/editar/${servidorId}`)
              }
