...
function MatriculaAlunoPage() {
  ...
  const [responsavelLegalParentesco, setResponsavelLegalParentesco] =
    useState("");
  ...
  const [currentPessoaAutorizadaNome, setCurrentPessoaAutorizadaNome] =
    useState("");
  const [
    currentPessoaAutorizadaParentesco,
  ...
  const validateCPF = (rawCpf) => {
    ...
    let sum = 0,
      remainder;
    for (let i = 1; i <= 9; i++)
      sum += parseInt(cpfCleaned.substring(i - 1, i)) * (11 - i);
    remainder = (sum * 10) % 11;
    if (remainder === 10 || remainder === 11) remainder = 0;
    if (remainder !== parseInt(cpfCleaned.substring(9, 10))) return false;
    sum = 0;
    for (let i = 1; i <= 10; i++)
      sum += parseInt(cpfCleaned.substring(i - 1, i)) * (12 - i);
    remainder = (sum * 10) % 11;
    if (remainder === 10 || remainder === 11) remainder = 0;
    ...
    if (!loading) {
      ...
      const fetchData = async () => {
        try {
          ...
          let schoolsList = schoolsSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
          }));
          ...
          const componentesSnapshot = await getDocs(
            collection(db, "componentes")
          );
          setAllComponentes(
            componentesSnapshot.docs.map((doc) => ({
              id: doc.id,
              ...doc.data(),
            }))
          );
          ...
          setErrorMessage("Erro ao carregar dados da página.");
        }
      };
      fetchData();
    }
      ...
      const fetchTurmas = async () => {
        try {
          ...
          const q = query(
            ...
            where("nivelEnsino", "==", matriculaNivelEnsino),
            where("anoSerie", "==", matriculaAnoSerie),
            orderBy("nomeTurma")
          );
    ...
    if (searchPessoaTerm.length >= 3) {
      const fetchSuggestions = async () => {
        try {
          ...
          if (
            !isNaN(searchPessoaTerm.replace(/\D/g, "")) &&
            searchPessoaTerm.replace(/\D/g, "").length >= 6
          ) {
            const qCpf = query(
              pessoasCol,
  ...
  const handleSelectPessoa = async (pessoa) => {
    ...
    try {
      ...
      if (pessoaDocSnap.exists()) {
        ...
        setResponsavelLegalNome(
          data.responsavelLegalNome || data.pessoaMae || ""
        );
  ...
  const handleAddPessoaAutorizada = () => {
    ...
    const isDuplicate = pessoasAutorizadas.some(
      (p) =>
        p.nome === currentPessoaAutorizadaNome &&
        p.parentesco === currentPessoaAutorizadaParentesco
    );
    if (isDuplicate) {
  ...
  const handlePhotoUpload = async (event) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setErrorMessage(
        "Formato de imagem não permitido. Use JPEG, JPG, PNG ou GIF."
      );
      return;
    }
    setFotoUpload(file);
    ...
    setSuccessMessage(
      "Foto selecionada. O upload ocorrerá ao cadastrar/salvar."
    );
    setErrorMessage("");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedPessoaId) {
      if (
        ...
        setErrorMessage(
          "Todos os campos obrigatórios em Dados Pessoais, Informações Familiares e Contato são necessários para cadastrar uma nova pessoa."
        );
        return;
      }
    ...
    if (
      ...
      setErrorMessage(
        "Os campos Escola, Nível de Ensino, Ano/Série, Turma, Data da Matrícula e Responsável Legal são obrigatórios."
      );
      return;
    }
    if (!codigoAluno && !selectedPessoaId) {
      setErrorMessage(
        "Código do Aluno é obrigatório para nova matrícula sem vinculação a pessoa existente."
      );
      return;
    }
    ...
    if (matriculaDependencia === "sim" && componentesDependencia.length === 0) {
      setErrorMessage(
        "Para matrícula de dependência, ao menos um componente deve ser selecionado."
      );
      return;
    }
    ...
    try {
      ...
      if (!selectedPessoaId) {
        ...
        if (password.length < 6) {
          setErrorMessage(
            "CPF muito curto para gerar senha automática (mínimo 6 dígitos)."
          );
          return;
        }
        ...
        setSuccessMessage(
          "Pessoa cadastrada e usuário criado automaticamente! Senha: " +
            password
        );
      } else {
      ...
      const matriculaData = {
        ...
        componentesDependencia:
          matriculaDependencia === "sim" ? componentesDependencia : [],
        ultimaAtualizacao: new Date(),
      };
      ...
      if (error.code === "auth/email-already-in-use") {
        ...
        msg =
          "Senha automática fraca. CPF da pessoa precisa ter no mínimo 6 dígitos para gerar a senha.";
      }
      setErrorMessage(msg);
    }
  };
  ...
  const availableComponentesParaSerie = allComponentes
    .filter((comp) => comp.serieAno === matriculaAnoSerie)
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  ...
                  placeholder="Deixe sempre em branco!"
              ...
              <div className="col-span-full md:col-span-2">
                <label
                  htmlFor="codigoSistemaEstadual"
                  className="block text-sm font-medium text-gray-700"
                >
                  Código da institução (quando houver)
                </label>
                <input
                  type="text"
                  id="codigoSistemaEstadual"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={codigoSistemaEstadual}
                  onChange={(e) => setCodigoSistemaEstadual(e.target.value)}
                  autoComplete="off"
                />
              </div>
              <div className="col-span-full md:col-span-2">
                ...
                {fotoURL && (
                  <div className="mt-2">
                    <img
                      ...
                      className="max-h-32 max-w-32 object-cover rounded-md"
                    />
                  ...
                  {pessoaSuggestions.map((pessoa) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-500">
                        {pessoa.cpf ? formatCPF(pessoa.cpf) : ""}
                      </span>
                    </li>
                  ))}
                </ul>
              )}
              {searchPessoaTerm.length >= 3 &&
                pessoaSuggestions.length === 0 && (
                  <p className="text-sm text-gray-500 mt-1">
                    Nenhuma pessoa encontrada. Preencha os campos abaixo para
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  id="rgNumero"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.rgNumero || formatRG(rgNumero)}
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  id="rgDataEmissao"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.rgDataEmissao || rgDataEmissao}
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  value={selectedPessoaData?.rgOrgaoEmissor || rgOrgaoEmissor}
                  onChange={(e) =>
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  id="rgEstado"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={selectedPessoaData?.rgEstado || rgEstado}
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={
                    selectedPessoaData?.nisPisPasep || formatNIS(nisPisPasep)
                  }
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={
                    selectedPessoaData?.carteiraSUS ||
                    formatCarteiraSUS(carteiraSUS)
                  }
              ...
              <div className="col-span-full md:col-span-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="col-span-full md:col-span-1">
                  ...
                  <select
                    id="certidaoTipo"
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                    value={selectedPessoaData?.certidaoTipo || certidaoTipo}
                ...
                <div className="col-span-full md:col-span-3">
                  ...
                  <input
                    ...
                    id="certidaoNumero"
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                    value={selectedPessoaData?.certidaoNumero || certidaoNumero}
                ...
                <div className="col-span-full md:col-span-1">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                    value={
                      selectedPessoaData?.certidaoDataEmissao ||
                      certidaoDataEmissao
                    }
                ...
                <div className="col-span-full md:col-span-3">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                    value={
                      selectedPessoaData?.certidaoCartorio || certidaoCartorio
                    }
                    onChange={(e) =>
                ...
                <div className="col-span-full md:col-span-2">
                  ...
                  <input
                    ...
                    id="certidaoCidade"
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                    value={selectedPessoaData?.certidaoCidade || certidaoCidade}
                    onChange={(e) =>
                ...
                <div className="col-span-full md:col-span-2">
                  ...
                  <input
                    ...
                    id="certidaoEstado"
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                    value={selectedPessoaData?.certidaoEstado || certidaoEstado}
                    onChange={(e) =>
              ...
              <div className="col-span-full md:col-span-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="col-span-full md:col-span-2">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                    value={
                      selectedPessoaData?.passaporteNumero || passaporteNumero
                    }
                    onChange={(e) =>
                ...
                <div className="col-span-full md:col-span-1">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                    value={
                      selectedPessoaData?.passaporteDataEmissao ||
                      passaporteDataEmissao
                    }
                ...
                <div className="col-span-full md:col-span-1">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                    value={
                      selectedPessoaData?.passaportePaisEmissor ||
            ...
            <div className="grid grid-cols-1 md:grid-cols-8 gap-4">
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  id="cep"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.cep || formatCEP(cep)}
              ...
              <div className="col-span-full md:col-span-6">
                ...
                <input
                  ...
                  id="rua"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={selectedPessoaData?.enderecoLogradouro || rua}
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  id="numero"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.enderecoNumero || enderecoNumero}
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={
                    selectedPessoaData?.enderecoComplemento ||
                    enderecoComplemento
                  }
                  onChange={(e) =>
              ...
              <div className="col-span-full md:col-span-4">
                ...
                <input
                  ...
                  id="bairro"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={selectedPessoaData?.enderecoBairro || enderecoBairro}
                  onChange={(e) =>
              ...
              <div className="col-span-full md:col-span-4">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={
                    selectedPessoaData?.municipioResidencia ||
                    municipioResidencia
                  }
                  onChange={(e) =>
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  id="paisResidencia"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={selectedPessoaData?.paisResidencia || paisResidencia}
                  onChange={(e) =>
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <select
                  id="zonaResidencia"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.zonaResidencia || zonaResidencia}
                  onChange={(e) => setZonaResidencia(e.target.value)}
                  disabled={!!selectedPessoaId}
                  autoComplete="off"
                >
                  <option value="urbana">Urbana</option>{" "}
                  <option value="urbana">Suburbana</option>{" "}
              ...
              <div className="col-span-full md:col-span-8">
                ...
                <select
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={
                    selectedPessoaData?.localizacaoDiferenciada ||
                    localizacaoDiferenciada
                  }
              ...
              <div className="col-span-full">
                ...
                <input
                  ...
                  id="pontoReferencia"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={selectedPessoaData?.pontoReferencia || pontoReferencia}
                  onChange={(e) =>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={
                    selectedPessoaData?.telefoneResidencial ||
                    formatTelefone(telefoneResidencial)
                  }
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  ...
                  id="celular"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.celular || formatTelefone(celular)}
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={
                    selectedPessoaData?.telefoneAdicional ||
                    formatTelefone(telefoneAdicional)
                  }
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  ...
                  id="emailContato"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={selectedPessoaData?.emailContato || emailContato}
          ...
          <div className="md:col-span-2 border p-4 rounded-lg bg-gray-50 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-4">
              <div className="col-span-full md:col-span-2">
                <label
                  htmlFor="currentPessoaAutorizadaNome"
                  className="block text-sm font-medium text-gray-700"
                >
                  Nome Autorizado
                </label>
                <input
                  type="text"
                  id="currentPessoaAutorizadaNome"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={currentPessoaAutorizadaNome}
                  onChange={(e) =>
                    setCurrentPessoaAutorizadaNome(e.target.value.toUpperCase())
                  }
                  autoComplete="off"
                />
              </div>
              <div className="col-span-full md:col-span-1">
                <label
                  htmlFor="currentPessoaAutorizadaParentesco"
                  className="block text-sm font-medium text-gray-700"
                >
                  Parentesco
                </label>
                <input
                  type="text"
                  id="currentPessoaAutorizadaParentesco"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={currentPessoaAutorizadaParentesco}
                  onChange={(e) =>
                    setCurrentPessoaAutorizadaParentesco(
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  ...
                  id="religiaoAluno"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={selectedPessoaData?.religiao || religiao}
                  onChange={(e) => setReligiao(e.target.value.toUpperCase())}
                  disabled={!!selectedPessoaId}
                  autoComplete="off"
                />
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <input
                  type="text"
                  id="beneficiosSociais"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={beneficiosSociais}
                  onChange={(e) =>
                    setBeneficiosSociais(e.target.value.toUpperCase())
                  }
                  autoComplete="off"
                />
              </div>
              <div className="col-span-full md:col-span-2">
                <label
                  htmlFor="deficienciasTranstornos"
                  className="block text-sm font-medium text-gray-700"
                >
                  Deficiências ou Transtornos (Escreva)
                </label>
                <input
                  type="text"
                  id="deficienciasTranstornos"
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={deficienciasTranstornos}
                  onChange={(e) =>
                    setDeficienciasTranstornos(e.target.value.toUpperCase())
                  }
                  autoComplete="off"
                />
              </div>
              <div className="col-span-full md:col-span-1">
                ...
                <select
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={alfabetizado}
                  onChange={(e) => setAlfabetizado(e.target.value)}
                  autoComplete="off"
                >
                  <option value="sim">Sim</option>{" "}
                  <option value="nao">Não</option>
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <select
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={emancipado}
                  onChange={(e) => setEmancipado(e.target.value)}
                  autoComplete="off"
                >
                  <option value="sim">Sim</option>{" "}
                  <option value="nao">Não</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
              <div className="col-span-full md:col-span-1">
                ...
                <select
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
                  value={utilizaTransporte}
                  onChange={(e) => setUtilizaTransporte(e.target.value)}
                  autoComplete="off"
                >
                  <option value="nao">Não</option>{" "}
                  <option value="sim">Sim</option>
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <input
                  ...
                  className="mt-1 block w-full p-2 border border-gray-300 rounded-md uppercase"
                  value={veiculoTransporte}
                  onChange={(e) =>
                    setVeiculoTransporte(e.target.value.toUpperCase())
                  }
                  disabled={utilizaTransporte === "nao"}
                  autoComplete="off"
                />
              </div>
              <div className="col-span-full md:col-span-4">
                <label
                  htmlFor="rotaTransporte"
                  className="block text-sm font-medium text-gray-700"
                >
                  Rota
                </label>
                <input
                  ...
                  onChange={(e) =>
                    setRotaTransporte(e.target.value.toUpperCase())
                  }
                  disabled={utilizaTransporte === "nao"}
                  autoComplete="off"
                />
          ...
          <div className="md:col-span-2 border p-4 rounded-lg bg-gray-50 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="col-span-full">
                ...
                <select
                  className="mt-1 block w-full p-2 border rounded-md"
                  value={matriculaEscolaId}
                  onChange={(e) => setMatriculaEscolaId(e.target.value)}
                  required
                >
              ...
              <div className="col-span-full">
                ...
                <select
                  className="mt-1 block w-full p-2 border rounded-md"
                  value={matriculaNivelEnsino}
                  onChange={(e) => setMatriculaNivelEnsino(e.target.value)}
                  required
                  disabled={!matriculaEscolaId}
                >
                  <option value="">Selecione o Nível</option>
                  {availableNiveisEnsino.map((n) => (
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <select
                  className="mt-1 block w-full p-2 border rounded-md"
                  value={matriculaAnoSerie}
                  onChange={(e) => setMatriculaAnoSerie(e.target.value)}
                  required
                  disabled={!matriculaNivelEnsino}
                >
                  <option value="">Selecione</option>
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <select
                  className="mt-1 block w-full p-2 border rounded-md"
                  value={matriculaTurmaId}
                  onChange={(e) => setMatriculaTurmaId(e.target.value)}
                  required
                  disabled={!matriculaAnoSerie}
                >
                  <option value="">Selecione</option>
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <select
                  className="mt-1 block w-full p-2 border rounded-md"
                  value={matriculaDependencia}
                  onChange={(e) => setMatriculaDependencia(e.target.value)}
                >
                  <option value="nao">Não</option>
                  <option value="sim">Sim</option>
                </select>
              ...
              {matriculaDependencia === "sim" && (
                <div className="col-span-full p-4 bg-yellow-50 border-l-4 border-yellow-400">
                  <label
                    htmlFor="componentesDependencia"
                    className="block text-sm font-medium text-gray-700 mb-1"
                  >
                    Componentes Curriculares em Dependência (máx. 3) *
                  </label>
                  <p className="text-xs text-gray-500 mb-2">
                    Segure Ctrl (ou Cmd em Mac) para selecionar mais de um.
                  </p>
                  <select
                  ...
                  >
                    {availableComponentesParaSerie.length > 0 ? (
                      ...
                      <option disabled>
                        Selecione uma Série/Ano para ver os componentes
                      </option>
                    )}
                  </select>
                </div>
          ...
          <div className="md:col-span-2">
            <h3 className="text-lg font-semibold text-gray-700 mb-2">
              Documentação (Uploads)
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full md:col-span-1">
                <label
                  htmlFor="documentosDiversos"
                  className="block text-sm font-medium text-gray-700"
                >
                  Documentos Diversos (jpg, png, pdf, gif)
                </label>
                <input
                  type="file"
                  ...
                  multiple
                  onChange={(e) =>
                    setDocumentosDiversos(Array.from(e.target.files))
                  }
                />
                {documentosDiversos.length > 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    Arquivos selecionados:{" "}
                    {documentosDiversos.map((f) => f.name).join(", ")}
                  </p>
                )}
              </div>
              <div className="col-span-full md:col-span-1">
                ...
                {laudoMedico && (
                  <p className="text-xs text-gray-500 mt-1">
                    Arquivo selecionado: {laudoMedico.name}
                  </p>
                )}
              </div>
              <p className="col-span-full text-xs text-red-500 mt-1">
                (Upload de arquivos requer Firebase Storage e Cloud Functions
                para segurança e persistência.)
              </p>
            </div>
          </div>
