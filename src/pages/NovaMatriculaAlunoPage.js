...
function NovaMatriculaAlunoPage() {
  ...
  useEffect(() => {
    if (nivelEnsino) {
      ...
      setAvailableSeries(
        seriesForLevel.filter((serie) => schoolSeries.includes(serie))
      );
    } else {
      setAvailableSeries([]);
    }
  ...
  const handleSalvar = async (e) => {
    ...
    if (isDependencia && componentesDependencia.length === 0) {
      setError(
        "Se a matrícula é de dependência, ao menos um componente curricular deve ser selecionado."
      );
      return;
    }
        ...
        <form onSubmit={handleSalvar} className="space-y-4">
          ...
          <div className="flex items-center">
            <input
              type="checkbox"
              id="dependencia"
              checked={isDependencia}
              onChange={(e) => setIsDependencia(e.target.checked)}
