...
function PessoaManagementPage() {
  ...
  const resetForm = () => {
    setCpf(''); setNomeCompleto(''); setNomeSocialAfetivo(''); setSexo('Nao Informado'); setEstadoCivil('Solteiro(a)'); setDataNascimento(''); setNacionalidade(''); setRaca('Nao Declarada'); setPovoIndigena(''); setReligiao(''); setNaturalidadeCidade(''); setNaturalidadeEstado(''); setFalecido('nao'); setPessoaPai(''); setTelefonePai(''); setEscolaridadePai('Nao Informado'); setProfissaoPai(''); setPessoaMae(''); setTelefoneMae(''); setEscolaridadeMae('Nao Informado'); setProfissaoMae(''); setRgNumero(''); setRgDataEmissao(''); setRgOrgaoEmissor(''); setRgEstado(''); setNisPisPasep(''); setCarteiraSUS(''); setCertidaoTipo('Nascimento'); setCertidaoEstado(''); setCertidaoCartorio(''); setCertidaoDataEmissao(''); setCertidaoNumero(''); setCertidaoCidade(''); setPassaporteNumero(''); setPassaportePaisEmissor(''); setPassaporteDataEmissao(''); setCep(''); setRua(''); setEnderecoNumero(''); setEnderecoComplemento(''); setEnderecoBairro(''); setMunicipioResidencia(''); setPaisResidencia('BRASIL'); setZonaResidencia('urbana'); setLocalizacaoDiferenciada(''); setPontoReferencia(''); setTelefoneResidencial(''); setCelular(''); setTelefoneAdicional(''); setEmailContato(''); setErrorMessage(''); setSuccessMessage(''); setEditingPessoa(null);
    setUserStatus('');
  };

  const filteredPessoas = pessoas.filter(pessoa => (pessoa.nomeCompleto && pessoa.nomeCompleto.toLowerCase().includes(searchTerm.toLowerCase())) || (pessoa.cpf && pessoa.cpf.includes(searchTerm)));
    ...
    if (!loading) {
      ...
      const fetchPessoas = async () => {
        try {
          ...
          const pessoaList = pessoaSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    if (searchTerm.length >= 3) {
      const fetchSuggestions = async () => {
        try {
          ...
          const q = query(collection(db, 'pessoas'), where('nomeCompleto', '>=', searchLower.toUpperCase()), where('nomeCompleto', '<=', searchLower.toUpperCase() + '\uf8ff'), orderBy('nomeCompleto'));
  ...
  useEffect(() => {
    const checkUserStatus = async () => {
      ...
      if (cpfCleaned.length === 11 && !editingPessoa) {
        ...
        const q = query(collection(db, 'users'), where('cpf', '==', cpfCleaned));
        const querySnapshot = await getDocs(q);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nomeCompleto || !dataNascimento || !emailContato || !municipioResidencia || !sexo || !estadoCivil || !nacionalidade || !raca || !pessoaMae) {
      setErrorMessage('Todos os campos obrigatórios em Dados Pessoais e Informações Familiares são necessários.');
      return;
    }
    
    const pessoaData = {
      cpf: cpfCleaned, nomeCompleto, nomeSocialAfetivo, sexo, estadoCivil, dataNascimento, nacionalidade, raca, povoIndigena, religiao, naturalidadeCidade, naturalidadeEstado, falecido: falecido === 'sim', pessoaPai, telefonePai, escolaridadePai, profissaoPai, pessoaMae, telefoneMae, escolaridadeMae, profissaoMae, rgNumero, rgDataEmissao, rgOrgaoEmissor, rgEstado, nisPisPasep, carteiraSUS, certidaoTipo, certidaoEstado, certidaoCartorio, certidaoDataEmissao, certidaoNumero, certidaoCidade, passaporteNumero, passaportePaisEmissor, passaporteDataEmissao, cep, enderecoLogradouro: rua, enderecoNumero, enderecoComplemento, enderecoBairro, municipioResidencia, paisResidencia, zonaResidencia, localizacaoDiferenciada, pontoReferencia, telefoneResidencial, celular, telefoneAdicional, emailContato,
    ...
    try {
      if (editingPessoa) {
        ...
        setPessoas(pessoas.map(p => p.id === editingPessoa.id ? { id: p.id, ...pessoaData } : p));
      } else {
        const docRef = await addDoc(collection(db, 'pessoas'), { ...pessoaData, dataCadastro: new Date() });
  ...
  if (!userData || !(userData.funcao && (userData.funcao.toLowerCase() === 'administrador' || userData.funcao.toLowerCase() === 'secretario'))) { return <div className="p-6 text-center text-red-500">Acesso Negado.</div>; }
  ...
  return (
    <div className="flex-grow p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-lg mx-auto md:max-w-4xl">
        <h2 className="text-2xl font-bold mb-6 text-gray-800 text-center">{editingPessoa ? 'Editar Pessoa Cadastrada' : 'Cadastrar Nova Pessoa'}</h2>
        ...
        <div className="relative mb-4 max-w-full mx-auto">
          <input type="text" placeholder="Buscar por nome" className="w-full p-2 border border-gray-300 rounded-md" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} autoComplete="off" />
          {searchTerm.length >= 3 && personSearchSuggestions.length > 0 && (
            <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-y-auto mt-1">
              ...
              {personSearchSuggestions.map(pessoa => (
                <li key={pessoa.id} className="p-2 cursor-pointer hover:bg-gray-200" onClick={() => handleSelectSuggestion(pessoa)}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="col-span-full md:col-span-1">
                ...
                <div className={`mt-1 flex items-center justify-center w-full h-10 p-2 border rounded-md text-center font-bold text-sm transition-colors ${userStatus === 'CADASTRADO' ? 'bg-red-100 text-red-800 border-red-300' : userStatus === 'CADASTRAR' ? 'bg-green-100 text-green-800 border-green-300' : 'bg-gray-100'}`}>
                  {userStatus || '-'}
                </div>
              </div>
              <div className="col-span-full md:col-span-2">
              ...
              <div className="col-span-full md:col-span-1">
                <label htmlFor="naturalidadeEstado" className="block text-sm font-medium text-gray-700">Naturalidade (UF) <span className="text-red-500">*</span></label>
         ...
         <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <select id="escolaridadeMae" className="mt-1 block w-full p-2 border border-gray-300 rounded-md" value={escolaridadeMae} onChange={(e) => setEscolaridadeMae(e.target.value)} autoComplete="off">
                  {escolaridadeOptions.map(option => (<option key={option} value={option}>{option}</option>))}
                </select>
              </div>
              <div className="col-span-full md:col-span-2">
          ...
          <div className="md:col-span-2 flex justify-end space-x-3 mt-4">
            {editingPessoa && (<button type="button" onClick={resetForm} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded">Cancelar Edição</button>)}
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">{editingPessoa ? 'Salvar Alterações' : 'Cadastrar Pessoa'}</button>
          </div>
        </form>
        <hr className="my-8" />
                ...
                <tr key={pessoa.id} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-3 px-6 text-center">
                    <div className="flex item-center justify-center space-x-2">
                      <button onClick={() => handleEdit(pessoa)} className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-full text-xs">Editar</button>
                      <button onClick={() => handleDelete(pessoa.id)} className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-full text-xs">Excluir</button>
                    </div>
                  </td>
                </tr>
              ))}
