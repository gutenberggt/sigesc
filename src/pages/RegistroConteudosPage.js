    ...
    const fetchInitialData = async () => {
      ...
      if (isProfessor) {
        ...
        if (!servidorSnapshot.empty) {
          ...
          const professorSchoolIds = [
            ...new Set(alocacoes.map((a) => a.schoolId)),
          ];
          setAvailableSchools(
  ...
  useEffect(() => {
    const fetchTurmas = async () => {
      ...
      if (isProfessor) {
        ...
        if (!servidorSnapshot.empty) {
          const alocacoes = servidorSnapshot.docs[0].data().alocacoes || [];
          const turmasIds = alocacoes.flatMap((a) =>
    ...
    try {
      ...
      if (typeof selectedPeriod.value === "number") {
        ...
        const bimestreDoc = await getDoc(
          doc(db, "bimestres", selectedPeriod.value)
        );
      ...
      for (
        ...
        if (horarioData[weekDayName] && horarioData[weekDayName].length > 0) {
          const aulasDoDia = horarioData[weekDayName]
            ...
            .filter(
              (aula) =>
                !showComponenteFilter || aula.startsWith(selectedComponente)
            );
          if (aulasDoDia.length > 0) {
            dias.push({ data: dataFormatada });
          }
        }
      }
      setDiasLetivos(dias);
      ...
      const conteudosQuery = query(
        ...
        where("componente", "==", selectedComponente)
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-10 gap-4 mb-6 p-4 border rounded-md bg-gray-50">
          ...
          <div className="md:col-span-3">
            ...
            <select
              ...
              className="p-2 border rounded-md w-full"
              disabled={!selectedYear || availablePeriods.length === 0}
            >
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {diasLetivos.map(({ data }) => (
              <div
                key={data}
                className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start border-b pb-4"
              >
            ...
            {!loading &&
              ...
              selectedPeriod && (
                <p className="text-center p-4 text-gray-500">
                  Nenhum dia letivo encontrado para o período e horário desta
                  turma.
                </p>
              )}
          </div>
