...
function RelatoriosPage() {
  ...
  useEffect(() => {
    const fetchTurmas = async () => {
      ...
      const q = query(collection(db, 'turmas'), where('schoolId', '==', selectedSchoolId));
    ...
    const fetchPeriods = async () => {
      ...
      if (isBimestral) {
        ...
        const meses = Array.from({length: 12}, (_, i) => {
            const month = i + 1;
            const monthName = new Date(selectedYear, i, 1).toLocaleString('pt-BR', { month: 'long' });
  ...
  const handleGenerateReport = () => {
    ...
    const componenteParam = showComponenteFilter ? selectedComponente : 'DIARIO';
    let url = '';
  ...
  return (
    <div className="p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-2xl mx-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Gerar Relatórios</h2>
        <div className="space-y-4">
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">1. Tipo de Relatório</label>
            <select value={reportType} onChange={e => setReportType(e.target.value)} className="p-2 border rounded-md w-full">
              ...
              <option value="notas" disabled>Relatório de Notas (em breve)</option>
              <option value="registros" disabled>Relatório de Registros (em breve)</option>
            </select>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">2. Ano Letivo</label>
            <input type="number" value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="p-2 border rounded-md w-full" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">3. Escola</label>
            <select value={selectedSchoolId} onChange={e => setSelectedSchoolId(e.target.value)} className="p-2 border rounded-md w-full">
              <option value="">Selecione a Escola</option>
              {availableSchools.map(s => <option key={s.id} value={s.id}>{s.nomeEscola}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">4. Turma</label>
            <select value={selectedTurmaId} onChange={e => setSelectedTurmaId(e.target.value)} className="p-2 border rounded-md w-full" disabled={!selectedSchoolId}>
          ...
          <div>
            ...
            <select value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)} className="p-2 border rounded-md w-full" disabled={!selectedTurmaId}>
          ...
          {showComponenteFilter && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">6. Componente Curricular</label>
              <select value={selectedComponente} onChange={e => setSelectedComponente(e.target.value)} className="p-2 border rounded-md w-full" disabled={!selectedTurmaId}>
          ...
          <div className="pt-4 flex justify-end">
            <button onClick={handleGenerateReport} className="bg-blue-600 text-white p-2 px-6 rounded-md hover:bg-blue-700">
              Gerar Relatório
            </button>
          </div>
        </div>
