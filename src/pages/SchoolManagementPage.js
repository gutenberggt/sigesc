...
function SchoolManagementPage() {
  ...
  const [orgaoVinculado, setOrgaoVinculado] = useState('Secretaria Municipal de Educação');
  ...
  const [localizacaoDiferenciada, setLocalizacaoDiferenciada] = useState('Não está em área de localização diferenciada');
  ...
  const resetForm = () => {
    setNomeEscola(''); setIntegral('Não'); setTipoEscola('Sede'); setEscolaPolo('');
    setCodigoINEP(''); setSituacaoFuncionamento('Ativa'); setDependenciaAdm('Municipal');
    setOrgaoVinculado('Secretaria Municipal de educação'); setRegulamentacaoAutorizacao('');
    setRua(''); setNumero(''); setComplemento(''); setBairro(''); setMunicipio(''); setCep('');
	setEstadoResidencia(''); setZonaLocalizacao('urbana'); setLocalizacaoDiferenciada('');
    setTelefoneContato(''); setEmailInstitucional(''); setSiteRedeSocial('');
    setGestores([{ inep: '', nome: '', cargo: 'Selecione' }]); setSelectedNiveisEnsino([]);
    setCurrentNivelEnsino(''); setSelectedAnosSeries([]); setCurrentAnoSerie('');
    setAvailableAnosSeries([]); setErrorMessage(''); setSuccessMessage(''); setEditingSchool(null);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const schoolData = {
      nomeEscola: nomeEscola.toUpperCase(), integral, tipoEscola,
      escolaPolo: tipoEscola === 'Anexa' ? escolaPolo.toUpperCase() : '', codigoINEP,
      situacaoFuncionamento, dependenciaAdm, orgaoVinculado,
      regulamentacaoAutorizacao: regulamentacaoAutorizacao.toUpperCase(),
      rua: rua.toUpperCase(), numero, complemento: complemento.toUpperCase(),
      bairro: bairro.toUpperCase(), municipio: municipio.toUpperCase(),
      cep: cep.replace(/\D/g, ''), EstadoResidencia: estadoResidencia.toUpperCase(),
      zonaLocalizacao, localizacaoDiferenciada,
      telefoneContato: telefoneContato.replace(/\D/g, ''), emailInstitucional, siteRedeSocial,
      gestores: gestores.map(g => ({ ...g, nome: g.nome.toUpperCase() })),
      niveisEnsino: selectedNiveisEnsino, anosSeriesAtendidas: selectedAnosSeries,
      anosLetivosFuncionamento: '', ultimaAtualizacao: new Date(),
    };
  ...
  const handleEdit = (school) => {
	...
	setOrgaoVinculado(school.orgaoVinculado || 'Secretaria Municipal de Educação');
    ...
    setSelectedNiveisEnsino(school.niveisEnsino || []); setSelectedAnosSeries(school.anosSeriesAtendidas || []);
    setErrorMessage(''); setSuccessMessage('');
  };
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-8 gap-4">
              ...
              <div className="col-span-full md:col-span-4">
                <label htmlFor="bairro" className="block text-sm font-medium text-gray-700">Bairro / Vila / Distrito</label>
              ...
              <div className="col-span-full md:col-span-2">
                ...
                <select id="zonaLocalizacao" className="mt-1 block w-full p-2 border border-gray-300 rounded-md" value={zonaLocalizacao} onChange={(e) => setZonaLocalizacao(e.target.value)} autoComplete="off">
                  <option value="urbana">Urbana</option>
                  <option value="rural">Rural</option>
                </select>
              </div>
              <div className="col-span-full md:col-span-8">
                <label htmlFor="localizacaoDiferenciada" className="block text-sm font-medium text-gray-700">Localização diferenciada</label>
                <select id="localizacaoDiferenciada" className="mt-1 block w-full p-2 border border-gray-300 rounded-md" value={localizacaoDiferenciada} onChange={(e) => setLocalizacaoDiferenciada(e.target.value)} autoComplete="off">
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full md:col-span-1">
                <label htmlFor="telefoneContato" className="block text-sm font-medium text-gray-700">Telefone</label>
              ...
              <div className="col-span-full">
                <label htmlFor="siteRedeSocial" className="block text-sm font-medium text-gray-700">Site ou Rede Social</label>
          ...
          <div className="md:col-span-2">
            ...
            {gestores.map((gestor, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-8 gap-4 mb-4 items-end">
                <div className="col-span-full md:col-span-1"><label>INEP</label><input type="text" value={gestor.inep} onChange={(e) => handleGestorChange(index, 'inep', e.target.value)} className="w-full p-2 border rounded"/></div>
                <div className="col-span-full md:col-span-4"><label>Nome</label><input type="text" value={gestor.nome} onChange={(e) => handleGestorChange(index, 'nome', e.target.value.toUpperCase())} className="w-full p-2 border rounded"/></div>
                <div className="col-span-full md:col-span-2"><label>Cargo</label><select value={gestor.cargo} onChange={(e) => handleGestorChange(index, 'cargo', e.target.value)} className="w-full p-2 border rounded"><option>Selecione</option><option>Diretor(a)</option><option>Vice-diretor(a)</option><option>Coordenador(a)</option><option>Secretário(a)</option></select></div>
                {gestores.length > 1 && <button type="button" onClick={() => handleRemoveGestor(index)} className="bg-red-500 text-white p-2 rounded">-</button>}
              </div>
            ))}
            <button type="button" onClick={handleAddGestor} className="text-sm text-blue-600 hover:text-blue-800">+ Adicionar Gestor</button>
          </div>

          <div className="md:col-span-2">
            <h3 className="text-lg font-semibold text-gray-700 mb-2">🎓 Oferta de Ensino</h3>
            <div>
              <label>Níveis de ensino <span className="text-red-500">*</span></label>
              <div className="flex"><select value={currentNivelEnsino} onChange={(e) => setCurrentNivelEnsino(e.target.value)} className="w-full p-2 border rounded-l"><option value="">Selecione</option>{niveisDeEnsinoList.map(n => <option key={n} value={n}>{n}</option>)}</select><button type="button" onClick={handleAddNivelEnsino} className="bg-blue-500 text-white p-2 rounded-r">+</button></div>
              <div className="flex flex-wrap gap-2 mt-2">{selectedNiveisEnsino.map(n => <span key={n} className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full">{n} <button onClick={() => handleRemoveNivelEnsino(n)} className="ml-1 text-red-500">x</button></span>)}</div>
            </div>
            <div className="mt-4">
              <label>Anos/séries atendidas <span className="text-red-500">*</span></label>
              <div className="flex"><select value={currentAnoSerie} onChange={(e) => setCurrentAnoSerie(e.target.value)} className="w-full p-2 border rounded-l" disabled={selectedNiveisEnsino.length === 0}><option value="">Selecione</option>{availableAnosSeries.map(s => <option key={s} value={s}>{s}</option>)}</select><button type="button" onClick={handleAddAnoSerie} className="bg-blue-500 text-white p-2 rounded-r" disabled={!currentAnoSerie}>+</button></div>
              <div className="flex flex-wrap gap-2 mt-2">{selectedAnosSeries.map(s => <span key={s} className="bg-green-100 text-green-800 px-2 py-1 rounded-full">{s} <button onClick={() => handleRemoveAnoSerie(s)} className="ml-1 text-red-500">x</button></span>)}</div>
            </div>
          </div>
          
          <div className="md:col-span-2 flex justify-end space-x-3 mt-4">
            {editingSchool && <button type="button" onClick={resetForm} className="bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded">Cancelar Edição</button>}
            <button type="submit" className="bg-blue-600 text-white font-bold py-2 px-4 rounded">{editingSchool ? 'Salvar Alterações' : 'Cadastrar Escola'}</button>
          </div>
        </form>
                ...
                <tr key={school.id} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-3 px-6 text-center">
                    <div className="flex item-center justify-center gap-2">
                      <button type="button" onClick={() => handleEdit(school)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded text-xs">Editar</button>
                      <button type="button" onClick={() => handleDelete(school.id)} className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded text-xs">Excluir</button>
                      <Link to={`/dashboard/escola/turmas/${school.id}`} className="bg-purple-500 hover:bg-purple-700 text-white font-bold py-1 px-3 rounded text-xs inline-block">Turmas</Link>
                    </div>
                  </td>
                </tr>
              ))}
