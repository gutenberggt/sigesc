...
function UserManagementPage() {
  ...
  const [permissionPersonSuggestions, setPermissionPersonSuggestions] =
    useState([]);
  const [selectedPermissionPersonId, setSelectedPermissionPersonId] =
    useState("");
    ...
    try {
      if (editingUser) {
        ...
        const newUser = {
          ...
          ultimaAtualizacao: new Date(),
          escolaId:
            userPermissions.length > 0 && userPermissions[0].escolaId
              ? userPermissions[0].escolaId
              : null,
          escolasIds: userPermissions
            .filter((p) => p.escolaId)
            .map((p) => p.escolaId),
          turmasIds: userPermissions
            .filter(
              (p) =>
                (p.permissao === "aluno" || p.permissao === "professor") &&
                p.pessoaId
            )
            .map((p) => p.pessoaId),
        };
        ...
        >
          ...
          <div className="md:col-span-2 flex flex-col md:flex-row gap-4">
            ...
            <div className="w-full md:w-1/3">
              <label
                ...
                className="block text-sm font-medium text-gray-700"
              >
                Função Primária <span className="text-red-500">*</span>
              </label>
              <select
                id="funcao"
                className="mt-1 block w-full p-2 border border-gray-300 rounded-md"
          ...
          <div className="md:col-span-2 border p-4 rounded-lg bg-gray-50 mt-4">
            <h3 className="text-lg font-semibold text-gray-700 mb-4">
              Permissões de Acesso por Unidade
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-4">
              <div>
                <label
                  htmlFor="currentPermission"
                  className="block text-sm font-medium text-gray-700"
                >
                  Permissão
                </label>
            ...
            {(currentPermission === "aluno" ||
              currentPermission === "professor") && (
              <div className="mb-4 relative">
                <label
                  htmlFor="searchPersonName"
                  className="block text-sm font-medium text-gray-700"
                >
                  {currentPermission === "aluno"
                    ? "Buscar Aluno"
                ...
                {selectedPermissionPersonId && (
                  <p className="text-sm text-gray-600 mt-1">
                    Pessoa selecionada: {searchPersonName}
                  </p>
                )}
              </div>
            )}
                    ...
                    >
                      <span>
                        <span className="font-medium text-blue-700">
                          {perm.permissao.charAt(0).toUpperCase() +
