...
function BimestresPage() {
  ...
  const fetchBimestres = useCallback(async () => {
    setIsLoading(true);
    try {
      const q = query(collection(db, 'bimestres'), orderBy('anoLetivo', 'desc'), orderBy('dataInicio', 'asc'));
      const querySnapshot = await getDocs(q);
      const bimestresList = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  return (
    <div className="p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-800 text-center">Gestão de Bimestres</h2>
            ...
            <tbody className="text-gray-700">
              {bimestres.map(bimestre => (
                <tr key={bimestre.id} className="border-b hover:bg-gray-100">
                  ...
                  <td className="py-3 px-6">{formatDateForDisplay(bimestre.dataInicio)}</td>
                  <td className="py-3 px-6">{formatDateForDisplay(bimestre.dataFim)}</td>
                  <td className="py-3 px-6">
                    <span className={`py-1 px-3 rounded-full text-xs ${bimestre.situacao === 'ABERTO' ? 'bg-green-200 text-green-700' : 'bg-red-200 text-red-700'}`}>
                      {bimestre.situacao}
                    </span>
                  </td>
