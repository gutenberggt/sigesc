...
function EventosPage() {
  ...
  return (
    <div className="p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-6xl mx-auto"> {/* Aumentado o max-w- para a tabela maior */}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white">
            ...
            <tbody className="text-gray-700">
              {(userLoading || loading) ? (
                <tr><td colSpan={isAdministrador ? "7" : "6"} className="text-center p-4">Carregando...</td></tr>
              ) : (
                filteredEventos.map(evento => (
                  <tr key={evento.id} className="border-b hover:bg-gray-100">
                    {/* ======================= INÍCIO DA ALTERAÇÃO ======================= */}
                    <td className="py-3 px-6">{formatDate(evento.data)}</td> {/* Mantido como Data Inicial */}
                    <td className="py-3 px-6">{formatDate(evento.dataFinal) || '-'}</td> {/* Novo campo Data Final */}
                    <td className="py-3 px-6">{evento.horario || '-'}</td> {/* Novo campo Horário */}
                    <td className="py-3 px-6">{evento.local || '-'}</td> {/* Novo campo Local */}
