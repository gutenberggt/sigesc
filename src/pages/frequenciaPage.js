...
function FrequenciaPage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      const allSchools = schoolsSnapshot.docs.map((docu) => ({
        id: docu.id,
        ...docu.data(),
      }));
      ...
      if (isProfessor) {
        ...
        setAvailableSchools(
          allSchools.filter((s) => professorSchoolIds.includes(s.id))
        );
      } else {
  ...
  useEffect(() => {
    const fetchTurmas = async () => {
      ...
      if (isProfessor && cachedServidorData) {
        const turmasIds = cachedServidorData.flatMap((a) =>
          a.funcoes.map((f) => f.turmaId)
        );
        if (turmasIds.length > 0) {
          turmasQuery = query(
            turmasQuery,
            where(documentId(), "in", [...new Set(turmasIds)])
          );
        } else {
          setAvailableTurmas([]);
      ...
      setAvailableTurmas(
        snapshot.docs.map((docu) => ({ id: docu.id, ...docu.data() }))
      );
      setFilters({ selectedTurmaId: "" });
    };
    fetchTurmas();
      ...
      Object.values(horarioData).forEach((aulasDoDia) => {
        if (Array.isArray(aulasDoDia)) {
          aulasDoDia.forEach((aula) => {
            if (aula) componentes.add(aula.split(" - ")[0]);
          });
        }
      });
          ...
          aloc.funcoes.forEach((f) => {
            if (f.turmaId === filters.selectedTurmaId) {
              (f.componentesCurriculares || []).forEach((c) =>
                componentesDoProfessor.add(c)
              );
            }
          });
        });
        componentes = new Set(
          [...componentes].filter((c) => componentesDoProfessor.has(c))
        );
      }
    ...
    const fetchPeriods = async () => {
      ...
      try {
        const q = query(
          ...
          where("anoLetivo", "==", filters.selectedYear.toString()),
          orderBy("dataInicio")
        );
        const snapshot = await getDocs(q);
        const bimestresList = snapshot.docs.map((docu) => ({
          id: docu.id,
          ...docu.data(),
        }));
        const isBimestral =
  ...
  const selectedTurmaObject = filters.selectedTurmaId
    ? availableTurmas.find((t) => t.id === filters.selectedTurmaId)
    : null;
  ...
  const handleFrequenciaChange = (alunoId, data, aulaId) => {
    setFrequenciaData((prev) => {
      ...
      const currentStatus =
        newState[alunoId][data][aulaId] === undefined
          ? null
          : newState[alunoId][data][aulaId];
  ...
  const loadDiarioData = useCallback(async () => {
    if (
      ...
      (showComponenteFilter && !filters.selectedComponente)
    ) {
      setAlunosDaTurma([]);
      setDiasLetivos([]);
      setFrequenciaData({});
      setOriginalFrequenciaData({});
      return;
    }
    setLoading(true);
    setError("");
    try {
      ...
      if (studentIds.length > 0) {
        ...
        alunos = alunosSnap.docs.map((docu) => ({
          id: docu.id,
          ...docu.data(),
        }));
      ...
      if (typeof filters.selectedPeriod.value === "number") {
        dataInicio = new Date(
          filters.selectedYear,
          filters.selectedPeriod.value - 1,
          1
        );
        dataFim = new Date(
          filters.selectedYear,
          filters.selectedPeriod.value,
          0
        );
      } else {
        const bimestreDoc = await getDoc(
          doc(db, "bimestres", filters.selectedPeriod.value)
        );
        if (bimestreDoc.exists()) {
          const bData = bimestreDoc.data();
          const [startYear, startMonth, startDay] = bData.dataInicio
            .split("-")
            .map(Number);
          dataInicio = new Date(startYear, startMonth - 1, startDay);
          const [endYear, endMonth, endDay] = bData.dataFim
            .split("-")
            .map(Number);
            ...
            const aulasDoDia = horarioData[weekDayName]
              ...
              .filter(
                (aula) =>
                  ...
                  aula.nome.startsWith(filters.selectedComponente)
              );
      ...
      frequenciaSnap.forEach((d) => {
        ...
        if (!freqData[data.alunoId][data.data])
          freqData[data.alunoId][data.data] = {};
      ...
      for (const alunoId of alunosSet) {
        ...
        for (const dataAula of datasSet) {
          ...
          const aulasSet = new Set([
            ...Object.keys(oldAulas),
            ...Object.keys(newAulas),
          ]);

          for (const aulaId of aulasSet) {
            const oldStatus =
              oldAulas[aulaId] === undefined ? null : oldAulas[aulaId];
            const newStatus =
              newAulas[aulaId] === undefined ? null : newAulas[aulaId];
            ...
            if (newStatus === null && oldStatus !== null) {
              ...
              batch.set(docRef, {
                ...
                componente: showComponenteFilter
                  ? filters.selectedComponente
                  : "GERAL",
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-full mx-auto">
        ...
        <div className="text-sm text-gray-600 mb-4 p-2 bg-gray-50 rounded-md">
          ...
          <span className="inline-flex items-center ml-4">
            <div className="w-4 h-4 rounded-full bg-green-500 mr-1"></div> P =
            Presente
          </span>
          <span className="inline-flex items-center ml-4">
            <div className="w-4 h-4 rounded-full bg-red-500 mr-1"></div> F =
            Falta
          </span>
          <span className="inline-flex items-center ml-4">
            <div className="w-4 h-4 rounded-full bg-yellow-400 mr-1"></div> FJ =
            Falta Justificada
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-10 gap-4 mb-6 p-4 border rounded-md bg-gray-50">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              onChange={(e) =>
                setFilters({ selectedYear: parseInt(e.target.value) })
              }
              className="p-2 border rounded-md w-full"
            />
          </div>
          ...
          <div className="md:col-span-3">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setFilters({
                  selectedPeriod:
                    availablePeriods.find((p) => p.value.toString() === v) ||
                    null,
                });
              }}
              className="p-2 border rounded-md w-full"
                    ...
                    {diasLetivos.map((dia) =>
                      dia.bloqueado ? (
                        ...
                        dia.aulas.map((aula, aulaIndex) => (
                          <td
                            key={`${dia.data}-${aula.id}`}
                            className="py-2 px-1 border text-center relative"
                          >
                            <div className="flex justify-center">
                              <FrequenciaCell
                                status={
                                  frequenciaData[aluno.id]?.[dia.data]?.[
                                    aula.id
                                  ] === undefined
                                    ? null
                                    : frequenciaData[aluno.id]?.[dia.data]?.[
                        ...
                        <td
                          key={dia.data}
                          className="py-2 px-1 border bg-gray-50 relative"
                        >
                          <div className="absolute top-0 bottom-0 -right-px w-0.5 bg-gray-400"></div>
                        </td>
                      )
