...
function HorarioPage() {
  ...
  const handleNumeroDeAulasChange = (novoNumero) => {
    ...
    const confirmReset = window.confirm("Alterar o número de aulas irá resetar o quadro de horários atual, incluindo os intervalos. Deseja continuar?");
    if (confirmReset) {
        setNumeroDeAulas(novoNumero);
        const defaultRows = Array.from({ length: novoNumero }, (_, i) => ({ type: 'aula', label: `${i + 1}ª Aula` }));
        setScheduleRows(defaultRows);
        const defaultHorarios = Array.from({ length: novoNumero }, () => ({ inicio: '', fim: '' }));
  ...
  useEffect(() => {
    ...
    scheduleRows.forEach((row, index) => {
        ...
        if (row.type === 'intervalo') {
            ...
            newHorario.intervalos[index] = horario.intervalos?.[index] || 'INTERVALO';
        }
    });
  ...
  const fetchDadosIniciais = useCallback(async () => {
    ...
    try {
      ...
      if (professoresDaTurma.length === 0) {
        console.warn("Nenhum professor encontrado no documento da turma. Usando busca de fallback na coleção 'servidores'.");
        const servidoresQuery = query(collection(db, 'servidores'), where('alocacoes', 'array-contains-any', [{funcoes: [{turmaId: turmaId}]}]));
        ...
        for (const servidorDoc of todosServidoresSnap.docs) {
          ...
          for (const aloc of servidorData.alocacoes) {
            if (aloc.schoolId === turmaData.schoolId && aloc.anoLetivo === turmaData.anoLetivo) {
              for (const funcao of aloc.funcoes) {
                if (funcao.turmaId === turmaId && funcao.funcao === 'Professor(a)') {
                  const pessoaDoc = await getDoc(doc(db, 'pessoas', servidorData.pessoaId));
                  const nomeCompletoProfessor = pessoaDoc.exists() ? pessoaDoc.data().nomeCompleto : 'Desconhecido';
                  const primeiroNomeProfessor = nomeCompletoProfessor.split(' ')[0];
      ...
      if (escolaData && escolaData.integral === 'Não') {
        ...
        componentesValidos = professoresDaTurma.filter(p => !componentesARemover.includes(p.subject));
      }
      
      const unique = [...new Map(componentesValidos.map(item => [item['value'], item])).values()];
      ...
      if (horarioSnap.exists()) {
        ...
        if (data.scheduleRows && data.scheduleRows.length > 0) {
            setNumeroDeAulas(data.scheduleRows.filter(r => r.type === 'aula').length);
        }
      } else {
        const defaultRows = Array.from({ length: 5 }, (_, i) => ({ type: 'aula', label: `${i + 1}ª Aula` }));
        setScheduleRows(defaultRows);
        setHorariosDasAulas(Array.from({ length: 5 }, () => ({ inicio: '', fim: '' })));
      }
  ...
  const handleInsertInterval = (index) => {
    ...
    newScheduleRows.splice(index + 1, 0, { type: 'intervalo', label: 'Intervalo' });
    ...
    <div className="p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <div>
                <h2 className="text-2xl font-bold text-gray-800">Quadro de Aulas</h2>
                {turma && (
            ...
            <div className="w-full max-w-xs">
                <label htmlFor="numeroAulas" className="block text-sm font-medium text-gray-700 text-right">Aulas por dia</label>
                <input 
                    type="number"
                    id="numeroAulas"
                    value={numeroDeAulas}
                    onChange={(e) => handleNumeroDeAulasChange(Math.max(1, Math.min(10, parseInt(e.target.value, 10) || 1)))}
                    min="1"
                    max="10"
                        ...
                        <tr key={rowIndex}>
                            <td className="p-1 border text-center font-semibold align-top">
                                <div className="flex items-center justify-center">
                                    <button onClick={() => handleInsertInterval(rowIndex)} className="text-green-500 hover:text-green-700 mr-2" title="Adicionar intervalo após esta aula">
                                        <FaPlusCircle />
                                    </button>
                                    <div>
                                        {row.label}
                                        <div className="flex gap-1 mt-1 justify-center">
                                            <input type="text" placeholder="hh:mm" maxLength="5" value={horariosDasAulas[rowIndex]?.inicio || ''} onChange={(e) => handleTimeChange(rowIndex, 'inicio', e.target.value)} className="w-16 p-1 text-center border rounded"/>
                                            <input type="text" placeholder="hh:mm" maxLength="5" value={horariosDasAulas[rowIndex]?.fim || ''} onChange={(e) => handleTimeChange(rowIndex, 'fim', e.target.value)} className="w-16 p-1 text-center border rounded"/>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            {diasDaSemana.map(dia => (
                                ...
                                <CustomDropdown
                                    ...
                                    onChange={(selectedValue) => handleHorarioChange(dia.id, rowIndex, selectedValue)}
                                    placeholder="-- Vago --"
                                />
                                </td>
                            ))}
                ...
                if (row.type === 'intervalo') {
                    return (
                        <tr key={rowIndex} className="bg-gray-100">
                            <td className="py-2 px-3 border text-center font-semibold">{row.label}</td>
                            <td colSpan={diasDaSemana.length} className="p-1 border">
                                <input 
                                    type="text"
                                    value={horario.intervalos?.[rowIndex] || 'INTERVALO'}
                                    onChange={(e) => handleIntervalChange(rowIndex, e.target.value)}
