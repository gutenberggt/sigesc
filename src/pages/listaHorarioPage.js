  ...
  useEffect(() => {
    const fetchTurmasParaNovo = async () => {
        if (escolaParaNovoHorario) {
            const q = query(collection(db, 'turmas'), where('schoolId', '==', escolaParaNovoHorario));
            const snapshot = await getDocs(q);
            setAvailableTurmasParaNovo(snapshot.docs.map(doc => ({id: doc.id, ...doc.data()})));
        } else {
  ...
  return (
    <div className="p-6">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Horários de Aulas</h2>

        <div className="mb-8 p-4 border-2 border-dashed rounded-md bg-gray-50">
          <h3 className="text-lg font-semibold text-gray-700 mb-2">Criar Novo Horário de Aula</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="md:col-span-1">
              <label className="block text-sm font-medium">Escola</label>
              <select value={escolaParaNovoHorario} onChange={e => setEscolaParaNovoHorario(e.target.value)} className="mt-1 p-2 w-full border rounded-md">
            ...
            <div className="md:col-span-1">
              ...
              <select value={turmaParaNovoHorario} onChange={e => setTurmaParaNovoHorario(e.target.value)} className="mt-1 p-2 w-full border rounded-md" disabled={!escolaParaNovoHorario}>
                <option value="">Selecione a Turma</option>
                {availableTurmasParaNovo.map(t => <option key={t.id} value={t.id}>{t.nomeTurma}</option>)}
              </select>
            </div>
            <div className="md:col-span-1">
              <button onClick={handleCreateHorario} disabled={!turmaParaNovoHorario} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded transition w-full disabled:bg-gray-400">
                Criar Horário
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 p-4 border rounded-md bg-gray-50">
          ...
          <select value={selectedNivel} onChange={e => setSelectedNivel(e.target.value)} className="p-2 border rounded-md" disabled={!selectedSchool}>
            <option value="">Filtrar por Nível</option>
            {availableNiveis.map(n => <option key={n} value={n}>{n}</option>)}
          </select>
          <select value={selectedSerie} onChange={e => setSelectedSerie(e.target.value)} className="p-2 border rounded-md" disabled={!selectedNivel}>
            <option value="">Filtrar por Série/Ano</option>
            {availableSeries.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={selectedTurma} onChange={e => setSelectedTurma(e.target.value)} className="p-2 border rounded-md" disabled={!selectedSerie}>
            <option value="">Filtrar por Turma</option>
            {availableTurmas.map(t => <option key={t.id} value={t.id}>{t.nomeTurma}</option>)}
          </select>
          <div className="col-span-full flex justify-end">
            <button onClick={handleClearFilters} className="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition">
              Limpar Filtros
            </button>
          </div>
        </div>
                ...
                filteredHorarios.map(h => (
                  <tr key={h.id} className="border-b hover:bg-gray-100">
                    ...
                    <td className="py-3 px-6 text-center">
                      <Link to={`/dashboard/calendario/horario/${h.turmaId}`} className="text-blue-600 hover:text-blue-800">
                        Ver / Editar
                      </Link>
                    </td>
                  </tr>
