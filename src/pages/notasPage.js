...
const NotaInput = ({ value, onChange, disabled }) => {
  ...
  const handleChange = (e) => {
    ...
    if (normalized === '') { setDisplay(''); return; }
    const maybeNumber = Number(normalized);
    if (Number.isNaN(maybeNumber)) { setDisplay(sanitized); return; }
  ...
  const handleBlur = () => {
    ...
    if (Number.isNaN(n)) { setDisplay(''); onChange(''); return; }
    if (n < 0) n = 0;
    if (n > 10) n = 10;
...
function NotasPage() {
  ...
  const acessoPermitido = ['professor', 'secretário', 'administrador'].includes(funcao);
  const canEdit = acessoPermitido && !isSubmitting;

  const selectedTurmaObject = availableTurmas.find(t => t.id === filters.selectedTurmaId);
  const isAnosFinais = selectedTurmaObject && selectedTurmaObject.nivelEnsino?.includes('ANOS FINAIS');
  const showComponenteFilter = isAnosFinais;

  // Escolas
  useEffect(() => {
    getDocs(collection(db, 'schools')).then(snap =>
      setAvailableSchools(snap.docs.map(docu => ({ id: docu.id, ...docu.data() })))
    );
  }, []);

  // Turmas
  useEffect(() => {
    if (!filters.selectedSchoolId) { setAvailableTurmas([]); setFilters({ selectedTurmaId: '', selectedComponente: '' }); return; }
    getDocs(query(collection(db, 'turmas'), where('schoolId', '==', filters.selectedSchoolId)))
      .then(snap => {
        setAvailableTurmas(snap.docs.map(docu => ({ id: docu.id, ...docu.data() })));
  ...
  useEffect(() => {
    const load = async () => {
      if (!filters.selectedTurmaId || !showComponenteFilter) { setAvailableComponentes([]); setFilters({ selectedComponente: '' }); return; }
      const horarioSnap = await getDoc(doc(db, 'horarios', filters.selectedTurmaId));
      if (!horarioSnap.exists()) { setAvailableComponentes([]); setFilters({ selectedComponente: '' }); return; }
      const horarioData = horarioSnap.data().horario;
      let comps = new Set();
      Object.values(horarioData || {}).forEach(aulas => {
        if (Array.isArray(aulas)) aulas.forEach(aula => aula && comps.add(aula.split(' - ')[0]));
      });
        ...
        userData.alocacoes.forEach(aloc => {
          aloc.funcoes?.forEach(f => {
            if (f.turmaId === filters.selectedTurmaId) {
              (f.componentesCurriculares || []).forEach(c => compsProf.add(c));
            }
          });
        });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const matriculasList = matriculasSnap.docs.map(docu => ({ id: docu.id, ...docu.data() }));

        // Pessoas (em lotes de 10)
        const pessoaIds = matriculasList.map(m => m.pessoaId).filter(id => typeof id === 'string' && id.trim() !== '');
        let alunosDetalhes = [];
        for (let i = 0; i < pessoaIds.length; i += 10) {
          const chunk = pessoaIds.slice(i, i + 10);
          const pessoasSnap = await getDocs(query(collection(db, 'pessoas'), where('__name__', 'in', chunk)));
          alunosDetalhes = [...alunosDetalhes, ...pessoasSnap.docs.map(docu => ({ id: docu.id, ...docu.data() }))];
        }
        ...
        const alunosCompletos = matriculasList
          .map(m => {
            ...
            return { ...m, nomeCompleto: pessoa?.nomeCompleto || pessoa?.nome || 'Sem nome' };
          })
    ...
    fetchData();
  }, [filters.selectedTurmaId, filters.selectedYear, filters.selectedComponente]);

  if (!acessoPermitido) {
    return <div className="p-6 text-red-600 font-semibold">Acesso negado. Esta página é restrita a professores, secretários e administradores.</div>;
  }
...
<div className="grid grid-cols-1 md:grid-cols-10 gap-4 mb-6 p-4 border rounded-md bg-gray-50">
  <div className="md:col-span-3">
    ...
    <select
      ...
      onChange={(e) => setFilters({ selectedSchoolId: e.target.value, selectedTurmaId: '', selectedComponente: '' })}
      className="p-2 border rounded-md w-full"
    >
      <option value="">Selecione a Escola</option>
      {availableSchools.map(s => (
        <option key={s.id} value={s.id}>{s.nomeEscola}</option>
      ))}
    </select>
  </div>
  ...
  <div className="md:col-span-3">
    ...
    <select
      ...
      onChange={(e) => setFilters({ selectedTurmaId: e.target.value, selectedComponente: '' })}
      className="p-2 border rounded-md w-full"
      disabled={!filters.selectedSchoolId}
    >
      <option value="">Selecione a Turma</option>
      {availableTurmas.map(t => (
        <option key={t.id} value={t.id}>{t.nomeTurma}</option>
      ))}
    </select>
  </div>
            ...
            <table className="min-w-full bg-white border-collapse">
              ...
              <tbody>
                {alunosDaTurma.map((aluno, index) => {
                  const row = notas[aluno.pessoaId] || { b1: '', b2: '', r1: '', b3: '', b4: '', r2: '' };
                  const { total, media } = calcularTotais(
                    row.b1 || 0, row.b2 || 0, row.r1 || 0, row.b3 || 0, row.b4 || 0, row.r2 || 0
                  );
                  return (
                    <tr key={aluno.id}>
                      <td className="border text-center">{index + 1}</td>
                      <td className="border">{aluno.nomeCompleto}</td>
                      <td className="border text-center"><NotaInput value={row.b1} onChange={(v) => handleNotaChange(aluno.pessoaId, 'b1', v)} disabled={!canEdit} /></td>
                      <td className="border text-center"><NotaInput value={row.b2} onChange={(v) => handleNotaChange(aluno.pessoaId, 'b2', v)} disabled={!canEdit} /></td>
                      <td className="border text-center"><NotaInput value={row.r1} onChange={(v) => handleNotaChange(aluno.pessoaId, 'r1', v)} disabled={!canEdit} /></td>
                      <td className="border text-center"><NotaInput value={row.b3} onChange={(v) => handleNotaChange(aluno.pessoaId, 'b3', v)} disabled={!canEdit} /></td>
                      <td className="border text-center">
                        <NotaInput
                          value={row.b4}
                          onChange={(v) => handleNotaChange(aluno.pessoaId, 'b4', v)}
                          disabled={!canEdit}
                        />
                      </td>
                      <td className="border text-center">
                        <NotaInput
                          value={row.r2}
                          onChange={(v) => handleNotaChange(aluno.pessoaId, 'r2', v)}
                          disabled={!canEdit}
                        />
                      </td>
